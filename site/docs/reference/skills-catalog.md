...
Hermes 在安装时会复制一个庞大的内置技能库到 `~/.hermes/skills/`。下面列出的每个技能都链接到专门的页面，其中包含完整的定义、设置和用法。

Hermes 也会在 `hermes update` 时同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.hermes/skills/` 目录树中缺失，它仍然随 Hermes 一起提供；使用 `hermes skills reset &lt;name&gt; --restore` 恢复。

如果某个技能不在这个列表中，但存在于仓库中，则目录由 `website/scripts/generate-skill-docs.py` 重新生成。
...
| [`apple-reminders`](/user-guide/skills/bundled/apple/apple-apple-reminders) | 通过 remindctl 管理 Apple Reminders：添加、列出、完成。 | `apple/apple-reminders` |
...
| [`macos-computer-use`](/user-guide/skills/bundled/apple/apple-macos-computer-use) | 在后台操控 macOS 桌面——截图、鼠标、键盘、滚动、拖拽——而不会抢占用户的鼠标、键盘焦点或 Space。适用于任何支持工具调用的模型。当 `computer_use` 工具被...时加载此技能。 | `apple/macos-computer-use` |
...
| [`hermes-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) | 配置、扩展或贡献给 Hermes Agent。 | `autonomous-ai-agents/hermes-agent` |
...
| [`comfyui`](/user-guide/skills/bundled/creative/creative-comfyui) | 使用 ComfyUI 生成图像、视频和音频——安装、启动、管理节点/模型、运行带参数注入的工作流。使用官方的 comfy-cli 进行生命周期管理，并通过直接的 REST/WebSocket API 执行。 | `creative/comfyui` |
...
| [`humanizer`](/user-guide/skills/bundled/creative/creative-humanizer) | 人性化文本：去除 AI 痕迹，加入真实语气。 | `creative/humanizer` |
...
| [`pixel-art`](/user-guide/skills/bundled/creative/creative-pixel-art) | 使用时代调色板（NES、Game Boy、PICO-8）的像素艺术。 | `creative/pixel-art` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`kanban-orchestrator`](/user-guide/skills/bundled/devops/devops-kanban-orchestrator) | 针对通过看板路由工作的编排器角色的分解剧本 + 反诱惑规则。“不要自己动手做”规则和基本生命周期会自动注入到每个看板工作者的系统提示中；此技能... | `devops/kanban-orchestrator` |
| [`kanban-worker`](/user-guide/skills/bundled/devops/devops-kanban-worker) | Hermes 看板工作者的陷阱、示例和边界情况。生命周期本身会作为 KANBAN_GUIDANCE（来自 agent/prompt_builder.py）自动注入到每个工作者的系统提示中；当你需要更深入的细节时，可以加载此技能... | `devops/kanban-worker` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`dogfood`](/user-guide/skills/bundled/dogfood/dogfood-dogfood) | Web 应用的探索性 QA：发现 Bug、证据、报告。 | `dogfood` |
...
| [`github-code-review`](/user-guide/skills/bundled/github/github-github-code-review) | 审查 PR：差异、通过 gh 或 REST 的内联评论。 | `github/github-code-review` |
...
| [`heartmula`](/user-guide/skills/bundled/media/media-heartmula) | HeartMuLa：类似 Suno 的歌词+标签歌曲生成。 | `media/heartmula` |
...
| [`youtube-content`](/user-guide/skills/bundled/media/media-youtube-content) | YouTube 转录文本转摘要、线程、博客。 | `media/youtube-content` |
...
| [`obliteratus`](/user-guide/skills/bundled/mlops/mlops-inference-obliteratus) | OBLITERATUS：通过差异均值法消除 LLM 拒绝回答。 | `mlops/inference/obliteratus` |
...
| [`weights-and-biases`](/user-guide/skills/bundled/mlops/mlops-evaluation-weights-and-biases) | W&B：记录 ML 实验、超参搜索、模型注册表、仪表板。 | `mlops/evaluation/weights-and-biases` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`obsidian`](/user-guide/skills/bundled/note-taking/note-taking-obsidian) | 在 Obsidian 仓库中读取、搜索、创建和编辑笔记。 | `note-taking/obsidian` |
<a id="productivity"></a>
## 生产力

| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`airtable`](/user-guide/skills/bundled/productivity/productivity-airtable) | 通过 curl 调用 Airtable REST API。支持记录的增删改查、过滤、upsert。 | `productivity/airtable` |
...
| [`linear`](/user-guide/skills/bundled/productivity/productivity-linear) | Linear：通过 GraphQL + curl 管理工单、项目、团队。 | `productivity/linear` |
| [`maps`](/user-guide/skills/bundled/productivity/productivity-maps) | 通过 OpenStreetMap/OSRM 进行地理编码、兴趣点、路径和时区查询。 | `productivity/maps` |
| [`nano-pdf`](/user-guide/skills/bundled/productivity/productivity-nano-pdf) | 通过 nano-pdf CLI（自然语言提示）编辑 PDF 的文本/拼写/标题。 | `productivity/nano-pdf` |
...
| [`powerpoint`](/user-guide/skills/bundled/productivity/productivity-powerpoint) | 创建、读取、编辑 .pptx 文件、幻灯片、备注和模板。 | `productivity/powerpoint` |
| [`teams-meeting-pipeline`](/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | 通过 Hermes CLI 操作 Teams 会议摘要流水线——汇总会议、检查流水线状态、重放作业、管理 Microsoft Graph 订阅。 | `productivity/teams-meeting-pipeline` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`godmode`](/user-guide/skills/bundled/red-teaming/red-teaming-godmode) | 破解 LLM：Parseltongue、GODMODE、ULTRAPLINIAN。 | `red-teaming/godmode` |
...
| [`blogwatcher`](/user-guide/skills/bundled/research/research-blogwatcher) | 通过 blogwatcher-cli 工具监控博客和 RSS/Atom 订阅。 | `research/blogwatcher` |
| [`llm-wiki`](/user-guide/skills/bundled/research/research-llm-wiki) | Karpathy 的 LLM Wiki：构建/查询互联的 Markdown 知识库。 | `research/llm-wiki` |
| [`polymarket`](/user-guide/skills/bundled/research/research-polymarket) | 查询 Polymarket：市场、价格、订单簿、历史数据。 | `research/polymarket` |
| [`research-paper-writing`](/user-guide/skills/bundled/research/research-research-paper-writing) | 撰写 NeurIPS/ICML/ICLR 的机器学习论文：从设计到投稿。 | `research/research-paper-writing` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`openhue`](/user-guide/skills/bundled/smart-home/smart-home-openhue) | 通过 OpenHue CLI 控制 Philips Hue 灯、场景和房间。 | `smart-home/openhue` |
<a id="social-media"></a>
## social-media

| 技能 | 描述 | 路径 |
|------|------|------|
| [`xurl`](/user-guide/skills/bundled/social-media/social-media-xurl) | 通过 xurl CLI 操作 X/Twitter：发布、搜索、私信、媒体、v2 API。 | `social-media/xurl` |
...
| [`hermes-agent-skill-authoring`](/user-guide/skills/bundled/software-development/software-development-hermes-agent-skill-authoring) | 编写仓库内 SKILL.md：frontmatter、校验器、结构。 | `software-development/hermes-agent-skill-authoring` |
...
| [`spike`](/user-guide/skills/bundled/software-development/software-development-spike) | 一次性实验，在开始构建前验证想法。 | `software-development/spike` |
| [`subagent-driven-development`](/user-guide/skills/bundled/software-development/software-development-subagent-driven-development) | 通过 delegate_task subagent 执行计划（两阶段审查）。 | `software-development/subagent-driven-development` |
...
| [`test-driven-development`](/user-guide/skills/bundled/software-development/software-development-test-driven-development) | TDD：强制 RED-GREEN-REFACTOR，测试先于代码。 | `software-development/test-driven-development` |
| [`writing-plans`](/user-guide/skills/bundled/software-development/software-development-writing-plans) | 编写实施计划：小任务、路径、代码。 | `software-development/writing-plans` |
