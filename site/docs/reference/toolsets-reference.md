...
工具集（Toolsets）是工具的命名集合，用于控制 Agent 可以执行的操作。它们是按平台、会话或任务配置工具可用性的主要机制。

## 工具集的工作原理

每个工具都严格属于一个工具集。当你启用某个工具集时，该集合中的所有工具对 Agent 均变为可用。工具集分为三种类型：

- **核心（Core）** — 一组逻辑相关的工具（例如，`file` 集合包含 `read_file`、`write_file`、`patch`、`search_files`）
- **复合（Composite）** — 为常见场景组合多个核心工具集（例如，`debugging` 集合包含文件、终端和 Web 工具）
...
toolsets:
  - hermes-cli          # CLI 默认配置
  # - hermes-telegram   # 覆盖 Telegram 网关配置
```
...
| `delegation` | `delegate_task` | 生成独立的子 Agent 实例以进行并行工作。 |
...
| `image_gen` | `image_generate` | 通过 FAL.ai 进行文本转图像生成。 |
| `memory` | `memory` | 跨会话的持久化记忆管理。 |
| `messaging` | `send_message` | 在会话内向其他平台（Telegram、Discord 等）发送消息。 |
| `moa` | `mixture_of_agents` | 通过 Agent 混合（Mixture of Agents）实现多模型共识。 |
...
| `session_search` | `session_search` | 搜索过往的对话会话。 |
...
这些工具集会展开为多个核心工具集，为常见场景提供便捷的快捷方式：

| 工具集 | 展开为 | 使用场景 |
|---------|-----------|----------|
| `debugging` | `patch`, `process`, `read_file`, `search_files`, `terminal`, `web_extract`, `web_search`, `write_file` | 调试会话 — 提供文件访问、终端和 Web 研究，不包含浏览器或任务委派开销。 |
| `safe` | `image_generate`, `mixture_of_agents`, `vision_analyze`, `web_extract`, `web_search` | 只读研究和媒体生成。不包含文件写入、终端访问或代码执行。适用于不受信任或受限的环境。 |
...
|---------|-------------------------------|
| `hermes-cli` | 完整工具集 — 包含全部 38 个工具，包括 `clarify`。交互式 CLI 会话的默认配置。 |
| `hermes-acp` | 移除了 `clarify`, `cronjob`, `image_generate`, `mixture_of_agents`, `send_message`, `text_to_speech` 以及 Home Assistant 工具。专注于 IDE 环境下的编码任务。 |
| `hermes-api-server` | 移除了 `clarify`, `send_message` 和 `text_to_speech`。包含其余所有工具 — 适用于无法进行用户交互的编程访问场景。 |
...
| `hermes-bluebubbles` | 同 `hermes-cli`。 |
...
每个配置的 MCP 服务器都会在运行时生成一个 `mcp-<server>` 工具集。例如，如果你配置了一个 `github` MCP 服务器，系统会创建一个包含该服务器暴露的所有工具的 `mcp-github` 工具集。
...
插件可以在初始化期间通过 `ctx.register_tool()` 注册自己的工具集。这些工具集会与内置工具集一起显示，并可以以相同方式启用/禁用。

### 自定义工具集

在 `config.yaml` 中定义自定义工具集，以创建特定项目的工具包：
...
`hermes tools` 命令提供了一个基于 curses 的 UI，用于按平台逐个开启或关闭工具。此操作在工具级别（比工具集更细粒度）进行，并持久化到 `config.yaml` 中。即使工具所属的工具集已启用，被禁用的工具也会被过滤掉。

另请参阅：[工具参考](./tools-reference.md)，获取单个工具及其参数的完整列表。
