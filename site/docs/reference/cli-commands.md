...
| `--continue [name]`, `-c [name]` | 恢复最近一次会话，或恢复最近一次匹配标题的会话。 |
| `--worktree`, `-w` | 在隔离的 git worktree 中启动，用于并行 Agent 工作流。 |
| `--yolo` | 跳过危险命令的确认提示。 |
| `--pass-session-id` | 将会话 ID 包含在 Agent 的系统提示词（system prompt）中。 |
...
| `hermes profile` | 管理配置方案（profile）——支持多个隔离的 Hermes 实例。 |
...
/model openrouter:anthropic/claude-sonnet-4  # 切回云端模型
```

提供商和基础 URL 的更改会自动持久化到 `config.yaml`。当从自定义端点切换离开时，过期的基础 URL 会被清除，以防止其泄露给其他提供商。
...
:::tip WSL 用户
请使用 `hermes gateway run` 而不是 `hermes gateway start` —— WSL 对 systemd 的支持并不稳定。可以使用 tmux 将其包装以实现持久化：`tmux new -s hermes 'hermes gateway run'`。详情请参阅 [WSL 常见问题解答](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
管理凭据池以实现同提供商的密钥轮换。完整文档请参阅 [凭据池](/user-guide/features/credential-pools)。
```bash
hermes auth                                              # 交互式向导
...
|------------|-------------|
| `list` | 显示已计划的任务。 |
| `create` / `add` | 从提示词创建计划任务，可通过多次使用 `--skill` 附加一个或多个技能。 |
...
| `--description` | 人类可读的描述。 |
| `--skills` | 为 Agent 运行加载的以逗号分隔的技能名称。 |
...
输出整个 Hermes 设置的紧凑纯文本摘要。专为在寻求支持时粘贴到 Discord、GitHub Issues 或 Telegram 中而设计 —— 无 ANSI 颜色，无特殊格式，仅包含数据。
...
| `--since <TIME>` | 显示从相对时间点开始的行：`30m`、`1h`、`2d` 等。支持 `s`（秒）、`m`（分）、`h`（时）、`d`（天）。 |
...
当 `--since` 处于活动状态时，不带可解析时间戳的行也会被包含（它们可能是多行日志条目的后续行）。当 `--level` 处于活动状态时，不带可检测级别的行也会被包含。
...
| `list` | 列出已安装的技能。 |
| `check` | 检查已安装的中心（hub）技能是否有上游更新。 |
| `update` | 在有可用更新时，重新安装带有上游变更的中心技能。 |
| `audit` | 重新扫描已安装的中心技能。 |
...
`--target-profile` 标志允许你在不切换配置的情况下管理另一个配置文件的 Honcho 设置。
...
| `mode [mode]` | 显示或设置回溯模式：`hybrid`、`context` 或 `tools`。省略参数则显示当前模式。 |
...
设置并管理外部记忆提供程序插件。可用提供程序：honcho, openviking, mem0, hindsight, holographic, retaindb, byterover, supermemory。同一时间只能激活一个外部提供程序。内置记忆（MEMORY.md/USER.md）始终处于活动状态。
...
请参阅 [MCP Config Reference](./mcp-config-reference.md)、[Use MCP with Hermes](../guides/use-mcp-with-hermes.md) 以及 [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server)。
...
统一插件管理 — 在一处管理通用插件、记忆提供程序和上下文引擎。不带子命令运行 `hermes plugins` 将打开一个包含两个部分的复合交互式屏幕：

- **General Plugins（通用插件）** — 用于启用/禁用已安装插件的多选复选框。
- **Provider Plugins（提供程序插件）** — 用于记忆提供程序和上下文引擎的单选配置。在类别上按 ENTER 键可打开单选列表。

| 子命令 | 描述 |
|------------|-------------|
| *(无)* | 复合交互式 UI — 通用插件开关 + 提供程序插件配置。 |
...
- `memory.provider` — 活动的记忆提供程序（为空则仅使用内置记忆）
- `context.engine` — 活动的上下文引擎（`"compressor"` 为内置默认值）
...
将你的 OpenClaw 设置迁移到 Hermes。它会读取 `~/.openclaw`（或自定义路径）并写入 `~/.hermes`。自动检测旧版目录名称（`~/.clawdbot`、`~/.moldbot`）和配置文件名（`clawdbot.json`、`moldbot.json`）。

| 选项 | 描述 |
|--------|-------------|
| `--dry-run` | 预览迁移内容，不执行任何写入操作。 |
...
迁移涵盖了 persona、memory、skills、模型提供商、消息平台、Agent 行为、会话策略、MCP 服务器、TTS 等 30 多个类别。项目要么被**直接导入**到 Hermes 的对应项中，要么被**归档**以供手动审查。

**直接导入：** SOUL.md、MEMORY.md、USER.md、AGENTS.md、skills（4 个源目录）、默认模型、自定义提供商、MCP 服务器、消息平台令牌和允许列表（Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Mattermost）、Agent 默认设置（推理力度、压缩、人工延迟、时区、沙箱）、会话重置策略、批准规则、TTS 配置、浏览器设置、工具设置、执行超时、命令允许列表、网关配置以及来自 3 个来源的 API 密钥。

**归档以供手动审查：** Cron 任务、插件、hooks/webhooks、内存后端（QMD）、skills 注册表配置、UI/身份、日志记录、多 Agent 设置、频道绑定、IDENTITY.md、TOOLS.md、HEARTBEAT.md、BOOTSTRAP.md。

**API 密钥解析**按优先级检查三个来源：配置值 → `~/.openclaw/.env` → `auth-profiles.json`。所有令牌字段均支持处理纯字符串、环境变量模板（`${VAR}`）和 SecretRef 对象。
...
管理配置文件 —— 即多个隔离的 Hermes 实例，每个实例拥有自己的配置、会话、skills 和主目录。
...
将 shell 补全脚本打印到标准输出。在你的 shell 配置文件中 source 该输出，即可实现 Hermes 命令、子命令和配置文件名的 tab 补全。
