...
:::caution
`hermes login` 已被移除。请使用 `hermes auth` 来管理 OAuth 凭证，使用 `hermes model` 来选择提供商，或使用 `hermes setup` 进行完整的交互式设置。
:::
...
输出你整个 Hermes 设置的紧凑、纯文本摘要。设计用于在请求支持时复制粘贴到 Discord、GitHub issues 或 Telegram——没有 ANSI 颜色，没有特殊格式，只有数据。
...
将你的 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入到 `~/.hermes`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件（`clawdbot.json`、`moltbot.json`）。
