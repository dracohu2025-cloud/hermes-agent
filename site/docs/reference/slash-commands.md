...
| `/stop` | 终止所有正在运行的后台进程 |
| `/queue &lt;prompt&gt;` (别名: `/q`) | 为下一轮排队一个提示（不中断当前 Agent 响应）。**注意：** `/q` 同时被 `/queue` 和 `/quit` 占用；最后注册的命令生效，因此实践中 `/q` 解析为 `/quit`。请明确使用 `/queue`。 |
| `/resume [name]` | 恢复一个之前命名的会话 |
| `/status` | 显示会话信息 |
| `/snapshot` (别名: `/snap`) | 创建或恢复 Hermes 配置/状态的状态快照（用法：/snapshot [create\|restore \<id\>\|prune]） |
| `/background &lt;prompt&gt;` (别名: `/bg`) | 在单独的后台会话中运行一个提示。Agent 独立处理你的提示——你当前的会话可以自由处理其他工作。任务完成后，结果会以一个面板形式显示。参见 [CLI 后台会话](/user-guide/cli#background-sessions)。 |
...
| `/verbose` | 循环切换工具进度显示：关闭 → 新工具 → 全部 → 详细。可以通过配置[为消息平台启用](#notes)。 |
...
用户定义的快捷命令将一个短别名映射到一个较长的提示。在 `~/.hermes/config.yaml` 中配置它们：
...
命令支持前缀匹配：输入 `/h` 解析为 `/help`，`/mod` 解析为 `/model`。当前缀有歧义（匹配多个命令）时，注册表顺序中的第一个匹配项胜出。完整的命令名称和已注册的别名始终优先于前缀匹配。
...
| `/resume [name]` | 恢复一个之前命名的会话。 |
| `/usage` | 显示令牌使用情况、估计成本细分（输入/输出）、上下文窗口状态和会话时长。 |
...
| `/background &lt;prompt&gt;` | 在单独的后台会话中运行一个提示。任务完成后，结果会发送回同一个聊天。参见[消息平台后台会话](/user-guide/messaging/#background-sessions)。 |
| `/plan [request]` | 加载捆绑的 `plan` 技能来编写 markdown 计划，而不是执行工作。计划保存在相对于活动工作空间/后端工作目录的 `.hermes/plans/` 下。 |
