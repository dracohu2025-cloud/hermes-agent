...
| `TERMINAL_DOCKER_FORWARD_ENV` | 需要显式转发到 Docker 终端会话的环境变量名称 JSON 数组。注意：技能声明的 `required_environment_variables` 会自动转发 —— 此变量仅用于任何技能都未声明的变量。 |
...
| `SUDO_PASSWORD` | 启用 sudo 且无需交互式提示 |
...
| `TERMINAL_SSH_PERSISTENT` | 为 SSH 覆盖持久化 shell 设置（默认：遵循 `TERMINAL_PERSISTENT_SHELL`） |
...
| `DISCORD_FREE_RESPONSE_CHANNELS` | 无需 @提及 即可响应的逗号分隔的频道 ID |
...
| `SIGNAL_ALLOWED_USERS` | 逗号分隔的 E.164 格式电话号码或 UUID |
...
| `TWILIO_PHONE_NUMBER` | Twilio E.164 格式电话号码（与电话技能共享） |
...
| `SMS_INSECURE_NO_SIGNATURE` | 设置为 `true` 以禁用 Twilio 签名验证（仅限本地开发 —— 不用于生产环境） |
| `SMS_ALLOWED_USERS` | 允许聊天的逗号分隔的 E.164 格式电话号码 |
...
| `WECOM_CALLBACK_ALLOWED_USERS` | 用于允许列表的逗号分隔的用户 ID |
...
| `GATEWAY_ALLOWED_USERS` | 允许跨所有平台的逗号分隔的用户 ID |
...
| `HERMES_STREAM_READ_TIMEOUT` | 流式传输套接字读取超时时间（秒）（默认：`120`）。对于本地提供者，会自动增加到 `HERMES_API_TIMEOUT`。如果本地 LLM 在生成长代码时超时，请增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 流式传输陈旧检测超时时间（秒）（默认：`180`）。对于本地提供者会自动禁用。如果在此时间窗口内没有收到数据块，则会触发连接终止。 |
...
| 变量 | 描述 |
|----------|-------------|
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时时间（秒）（默认：`600`）。Agent 在主动调用工具或接收流式令牌时可以无限期运行——此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 定时任务附加的预运行脚本超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本（例如，用于反机器人定时的随机延迟）覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
完整详情请参阅[备用提供者](/user-guide/features/fallback-providers)。
...
| `only` | 允许的提供者标识列表（例如，`["anthropic", "google"]`） |
| `ignore` | 要跳过的提供者标识列表 |
| `order` | 按顺序尝试的提供者标识列表 |
