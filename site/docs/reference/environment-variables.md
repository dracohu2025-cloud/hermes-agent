...
| `HERMES_CRON_SCRIPT_TIMEOUT` | 定时任务附加的预运行脚本超时时间（秒）（默认：`120`）。为需要更长时间执行的脚本（例如，用于反机器人计时的随机延迟）覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
...
上下文压缩只能通过 `config.yaml` 中的 `compression` 部分配置——没有对应的环境变量。
...
主模型备用配置只能通过 `config.yaml` 进行——没有对应的环境变量。添加一个包含 `provider` 和 `model` 键的 `fallback_model` 部分，以便在主模型遇到错误时启用自动故障转移。
