...
| `HERMES_HOME` | 覆盖 Hermes 配置目录 (默认: `~/.hermes`)。同时限定网关 PID 文件和 systemd 服务名称，以便多个安装可以同时运行 |
...
| `HERMES_NOUS_MIN_KEY_TTL_SECONDS` | 重新签发前的最小 Agent 密钥生存时间（默认: 1800 = 30分钟） |
...
| `FIRECRAWL_BROWSER_TTL` | Firecrawl 浏览器会话生存时间（秒）（默认: 300） |
...
| `SUPERMEMORY_API_KEY` | 具有个人资料召回和会话摄入功能的语义长期记忆 ([supermemory.ai](https://supermemory.ai)) |
...
| `TERMINAL_DOCKER_FORWARD_ENV` | 要显式转发到 Docker 终端会话的环境变量名称 JSON 数组。注意：技能声明的 `required_environment_variables` 会自动转发 —— 你只需要为任何技能都未声明的变量设置此项。 |
...
| `MATRIX_RECOVERY_KEY` | 设备密钥轮换后用于跨设备签名验证的恢复密钥。建议用于启用了跨设备签名的 E2EE 设置。 |
...
| `GATEWAY_ALLOWED_USERS` | 在所有平台上都允许的逗号分隔的用户 ID |
...
| `HERMES_STREAM_STALE_TIMEOUT` | 流式传输陈旧检测超时时间（秒）（默认：`180`）。对于本地提供商会自动禁用。如果在此时间窗口内没有收到数据块，则触发连接终止。 |
...
| 变量 | 描述 |
|----------|-------------|
| `HERMES_CRON_TIMEOUT` | 定时任务 Agent 运行的不活动超时时间（秒）（默认：`600`）。当 Agent 主动调用工具或接收流式令牌时，可以无限期运行——此设置仅在空闲时触发。设置为 `0` 表示无限制。 |
| `HERMES_CRON_SCRIPT_TIMEOUT` | 附加到定时任务的预运行脚本的超时时间（秒）（默认：`120`）。对于需要更长时间执行的脚本（例如，用于反机器人计时的随机延迟）可以覆盖此值。也可以通过 `config.yaml` 中的 `cron.script_timeout_seconds` 配置。 |
