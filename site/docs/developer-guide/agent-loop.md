...
description: "AIAgent 执行、API 模式、工具、回调与回退行为的详细说明"
---
...
### 顺序执行与并发执行 {#sequential-vs-concurrent}
...
- 每个 Agent 拥有自己的预算。子 Agent 拥有独立的预算，上限为 `delegation.max_iterations`（默认 50）——父 Agent 与子 Agent 的总迭代次数可以超过父 Agent 的上限
...
### 压缩期间发生什么 {#what-happens-during-compression}
