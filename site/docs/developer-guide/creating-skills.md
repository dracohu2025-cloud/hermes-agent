...
## 应该做成技能还是工具？ {#should-it-be-a-skill-or-a-tool}
...
### 条件技能激活 {#conditional-skill-activation}
...
| 字段 | 行为 |
|-------|----------|
| `requires_toolsets` | 当**任意**列出的工具集**不可用**时，技能**隐藏** |
| `requires_tools` | 当**任意**列出的工具**不可用**时，技能**隐藏** |
| `fallback_for_toolsets` | 当**任意**列出的工具集**可用**时，技能**隐藏** |
| `fallback_for_tools` | 当**任意**列出的工具**可用**时，技能**隐藏** |

**`fallback_for_*` 的使用场景：** 创建一个在主工具不可用时作为替代方案的技能。例如，一个 `duckduckgo-search` 技能设置了 `fallback_for_tools: [web_search]`，仅当 web 搜索工具（需要 API 密钥）未配置时才会显示。
**`requires_*` 的使用场景：** 创建一个只有在特定工具存在时才有意义的技能。例如，一个带有 `requires_toolsets: [web]` 的网页抓取工作流技能，在网页工具被禁用时不会使提示变得杂乱。
...
当技能需要 API 密钥或令牌时，请使用 `required_environment_variables`。缺失的值**不会**隐藏该技能使其不被发现。相反，当技能在本地 CLI 中加载时，Hermes 会安全地提示用户提供这些值。
```yaml
required_environment_variables:
  - name: TENOR_API_KEY
    prompt: Tenor API 密钥
...
当你的技能被加载时，任何已声明的 `required_environment_variables` 中已设置的变量，都会**自动透传**到 `execute_code` 和 `terminal` 沙箱中——包括 Docker 和 Modal 等远程后端。你的技能脚本可以直接访问 `$TENOR_API_KEY`（或在 Python 中使用 `os.environ["TENOR_API_KEY"]`），用户无需额外配置。详情请参阅[环境变量透传](/user-guide/security#environment-variable-passthrough)。
:::
...
技能可以声明非机密设置，这些设置存储在 `config.yaml` 的 `skills.config` 命名空间下。与环境变量（作为机密存储在 `.env` 中）不同，配置设置用于路径、偏好设置和其他非敏感值。
...
1. **存储：** 值会被写入 `config.yaml` 中 `skills.config.&lt;key&gt;` 路径下：
   ```yaml
   skills:
     config:
       myplugin:
...
对于 API 密钥、令牌和其他**机密信息**（存储在 `~/.hermes/.env` 中，永远不会展示给模型），请使用 `required_environment_variables`。对于**路径、偏好设置和非敏感设置**（存储在 `config.yaml` 中，可在配置展示中看到），请使用 `config`。
:::

### 凭证文件要求（OAuth 令牌等） {#credential-file-requirements-oauth-tokens-etc}

使用 OAuth 或基于文件的凭证的技能可以声明需要挂载到远程沙箱中的文件。这适用于以**文件**形式存储的凭证（而非环境变量）——通常是设置脚本生成的 OAuth 令牌文件。
```yaml
...
## 技能指南 {#skill-guidelines}

### 无外部依赖 {#no-external-dependencies}

优先使用 stdlib Python、curl 和现有的 Hermes 工具（`web_extract`、`terminal`、`read_file`）。如果需要依赖项，请在技能中记录安装步骤。
...
#### 从 SKILL.md 中引用打包脚本 {#referencing-bundled-scripts-from-skill-md}
...
#### 内联 Shell 代码片段（可选启用） {#inline-shell-snippets-opt-in}
...
此功能**默认关闭**——SKILL.md 中的任何代码片段都会在主机上未经批准直接运行，因此请仅对您信任的技能来源启用它：
...
内置技能（位于 `skills/` 目录）随每个 Hermes 安装包一起提供。它们应该**对大多数用户有广泛用途**：
