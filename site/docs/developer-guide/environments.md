...
2. **基准测试** — 在标准化的智能体基准测试上评估模型
3. **数据生成** — 从智能体运行轨迹生成 SFT 训练数据

这三者共享同一个核心：一个定义任务、运行智能体循环并对输出进行评分的**环境**类。
...
- **智能体循环集成** — `collect_trajectory()` 运行 `HermesAgentLoop` 并对结果评分
...
对于**阶段 2**（VLLM ManagedServer），服务器返回原始文本，没有结构化的工具调用。`environments/tool_call_parsers/` 中的客户端解析器从原始输出中提取 `tool_calls`：
...
|---|---|
| **测试内容** | 数百轮决策中的多轮战略连贯性 |
| **评分** | 复合评分：`0.5 × survival + 0.5 × normalised_funds` |
| **沙盒** | 本地终端（无需 Modal） |
| **工具** | 仅 `terminal` |
...
4.  **实现** `rollout_and_score_eval(eval_item)` — 每个任务项的智能体循环 + 评分
...
| `max_agent_turns` | `int` | `30` | 每次轨迹的最大 LLM 调用次数 |
| `agent_temperature` | `float` | `1.0` | 采样温度 |
| `system_prompt` | `str` | `None` | 给智能体的系统提示 |
...
| `tool_call_parser` | `str` | `"hermes"` | 用于阶段 2 原始输出的解析器 |
...
YAML 值会覆盖 `config_init()` 的默认值。命令行参数会覆盖 YAML 值：
...
- `tinker-atropos` 子模块（位于代码库的 `tinker-atropos/` 目录下）
...
├── tool_context.py             # 奖励函数使用的每轨迹工具访问上下文
