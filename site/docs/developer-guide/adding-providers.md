...
### 每个内置 Provider 必选 {#required-for-every-built-in-provider}
...
### 原生 / 非 OpenAI Provider 额外必选 {#additional-for-native-non-openai-providers}

10. `agent/<provider>_adapter.py`
11. `run_agent.py`
12. 如果需要 Provider SDK，则需修改 `pyproject.toml`

## 第 1 步：选择一个规范的 Provider ID {#step-1-pick-one-canonical-provider-id}
...
## 第 5 步：在 `hermes_cli/main.py` 中关联 CLI {#step-5-wire-the-cli-in-hermescli-main-py}
...
## 第 7 步：如果 Provider 是原生的，添加适配器和 `run_agent.py` 支持 {#step-7-if-the-provider-is-native-add-an-adapter-and-runagent-py-support}
...
### Prompt 缓存和提供商特定的请求字段 {#prompt-caching-and-provider-specific-request-fields}
...
## 第 9 步：实机验证 {#step-9-live-verification}
...
### 1. 将提供商添加到 auth 但未添加到模型解析 {#1-adding-the-provider-to-auth-but-not-to-model-parsing}
...
### 3. 假设必须添加内置提供商 {#3-assuming-a-built-in-provider-is-required}
...
### 5. 隐藏在 `run_agent.py` 中的原生提供商分支 {#5-native-provider-branches-hiding-in-runagent-py}

搜索 `api_mode` 和 `self.client.`。不要假设显式的请求路径是唯一的路径。

### 6. 向其他提供商发送仅限 OpenRouter 的参数 {#6-sending-openrouter-only-knobs-to-other-providers}

像提供商路由（provider routing）之类的字段仅属于支持它们的提供商。

### 7. 更新了 `hermes model` 但未更新 `hermes setup` {#7-updating-hermes-model-but-not-hermes-setup}

这两个流程都需要了解该提供商。

## 实现时的良好搜索目标 {#good-search-targets-while-implementing}
