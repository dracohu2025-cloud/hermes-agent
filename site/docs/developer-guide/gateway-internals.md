...
| `gateway/delivery.py` | 向目标平台/通道投递出站消息 |
...
| `gateway/status.py` | 用于配置文件作用域网关实例的令牌锁管理 |
...
2. **第 2 级 — 网关运行器** (`gateway/run.py`)：检查 `_running_agents`。拦截特定命令（`/stop`、`/new`、`/queue`、`/status`、`/approve`、`/deny`）并适当地路由它们。其他所有消息都会触发 `running_agent.interrupt()`。
...
与 CLI（使用带有硬编码默认值的 `load_cli_config()`）不同，网关通过 YAML 加载器直接读取 `config.yaml`。这意味着存在于 CLI 默认字典中但不在用户配置文件中的配置键，在 CLI 和网关之间的行为可能不同。
...
定时任务投递**不会**镜像到网关会话历史中——它们只存在于自己的定时任务会话中。这是一个深思熟虑的设计选择，以避免违反消息交替规则。
