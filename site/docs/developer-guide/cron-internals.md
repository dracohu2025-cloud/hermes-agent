...
cron 子系统提供定时任务执行功能——从简单的一次性延迟任务，到带有技能注入和跨平台交付的、基于 cron 表达式的周期性任务。
...
| `cron/scheduler.py` | 调度器循环——检测到期任务、执行、重复次数跟踪 |
| `tools/cronjob_tools.py` | 面向模型的 `cronjob` 工具注册和处理程序 |
| `gateway/run.py` | 网关集成——在长期运行循环中进行 cron 滴答 |
...
| **时间间隔** | `every 2h`, `every 30m` | 周期性，按固定间隔触发 |
...
任务存储在 `~/.hermes/cron/jobs.json` 中，采用原子写入语义（先写入临时文件，然后重命名）。每个任务记录包含：
...
在网关模式下，调度器滴答被集成到网关的主事件循环中。网关在其定期维护周期中调用 `scheduler.tick()`，该周期与消息处理并行运行。

在 CLI 模式下，cron 任务仅在运行 `hermes cron` 命令时或在活跃的 CLI 会话期间触发。
...
- 没有之前运行的对话历史
- 不记得之前的 cron 执行（除非持久化到内存/文件）
- 提示词必须是自包含的——cron 任务不能询问澄清性问题
- `cronjob` 工具集被禁用（递归防护）
...
这使得可重用、经过测试的工作流成为可能，而无需将完整指令粘贴到 cron 提示词中。例如：
...
-   **凭证池** — 使用解析出的运行时提供商名称，通过 `agent.credential_pool` 中的 `load_pool(provider)` 加载。仅在池中有凭证时传递（`pool.has_credentials()`）。这允许在 429/速率限制错误时进行同提供商密钥轮换。

这镜像了网关的行为——没有它，cron Agent 在遇到速率限制时会失败，而不会尝试恢复。
...
默认情况下（`cron.wrap_response: true`），cron 交付会包装以下内容：
-   标识 cron 任务名称和任务的标题
-   注明 Agent 在对话中看不到已交付消息的页脚

cron 响应中的 `[SILENT]` 前缀会完全抑制交付——适用于只需要写入文件或执行副作用的任务。

### 会话隔离

Cron 交付*不会*镜像到网关会话的对话历史中。它们仅存在于 cron 任务自身的会话中。这防止了目标聊天对话中出现消息交替违规。
...
调度器使用基于文件的锁定机制，防止重叠的定时触发重复执行同一批到期任务。这在网关模式下尤为重要，因为如果前一次触发执行时间超过了触发间隔，多个维护周期可能会重叠。
