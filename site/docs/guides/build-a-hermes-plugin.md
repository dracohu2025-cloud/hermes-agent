...
sidebar_label: "构建插件"
title: "构建 Hermes 插件"
description: "通过工具、钩子、数据文件和技能构建完整 Hermes 插件的分步指南"
---

# 构建 Hermes 插件

本指南将带你从零开始构建一个完整的 Hermes 插件。完成后，你将拥有一个包含多个工具、生命周期钩子、内置数据文件和打包技能的插件——涵盖了插件系统支持的所有功能。
...
name: calculator
version: 1.0.0
description: 数学计算器 — 计算表达式并进行单位换算
provides_tools:
  - calculate
  - unit_convert
provides_hooks:
...
这会告诉 Hermes：“我是一个名为 calculator 的插件，我提供工具和钩子。” `provides_tools` 和 `provides_hooks` 字段列出了插件所注册的内容。
...
创建 `schemas.py` —— 这是 LLM 读取并决定何时调用你的工具的依据：
...
**为什么模式很重要：** `description` 字段是 LLM 决定何时使用你的工具的关键。请明确描述它的功能及使用场景。`parameters` 定义了 LLM 需要传递的参数。
...
def calculate(args: dict, **kwargs) -> str:
    ...
    2. 执行任务
    3. 返回 JSON 字符串 — 必须始终返回，即使出错时也是如此
...
2. **返回值：** 始终为 JSON 字符串。成功和错误情况均如此。
3. **禁止抛出异常：** 捕获所有异常，并返回错误 JSON。
4. **接受 `**kwargs`：** Hermes 未来可能会传递额外的上下文。

## 第 5 步：编写注册逻辑

创建 `__init__.py` —— 这将模式与处理程序连接起来：
...
如果未设置 `WEATHER_API_KEY`，插件将被禁用并显示清晰的消息。不会崩溃，Agent 中也不会报错 —— 只是显示“Plugin weather disabled (missing: WEATHER_API_KEY)”。

当用户运行 `hermes plugins install` 时，系统会**交互式提示**输入任何缺失的 `requires_env` 变量。值会自动保存到 `.env` 中。
...
| `url` | 否 | 获取凭据的地址 |
| `secret` | 否 | 如果为 `true`，输入将被隐藏（像密码字段一样） |
...
每个钩子都在 **[事件钩子参考](/user-guide/features/hooks#plugin-hooks)** 中有完整记录 —— 包括回调签名、参数表、触发时机和示例。以下是摘要：
...
| [`post_llm_call`](/user-guide/features/hooks#post_llm_call) | 每轮一次，在工具调用循环之后（仅限成功轮次） | `session_id: str, user_message: str, assistant_response: str, conversation_history: list, model: str, platform: str` | 忽略 |
| [`on_session_start`](/user-guide/features/hooks#on_session_start) | 创建新会话（仅限第一轮） | `session_id: str, model: str, platform: str` | 忽略 |
| [`on_session_end`](/user-guide/features/hooks#on_session_end) | 每个 `run_conversation` 调用结束 + CLI 退出 | `session_id: str, completed: bool, interrupted: bool, model: str, platform: str` | 忽略 |
| [`pre_api_request`](/user-guide/features/hooks#plugin-hooks) | 向 LLM 提供商发送每个 HTTP 请求之前 | `method: str, url: str, headers: dict, body: dict` | 忽略 |
| [`post_api_request`](/user-guide/features/hooks#plugin-hooks) | 从 LLM 提供商收到每个 HTTP 响应之后 | `method: str, url: str, status_code: int, response: dict` | 忽略 |

大多数钩子都是“触发即忘”的观察者 —— 它们的返回值会被忽略。例外是 `pre_llm_call`，它可以向对话中注入上下文。

所有回调都应接受 `**kwargs` 以保持向前兼容性。如果钩子回调崩溃，它会被记录并跳过。其他钩子和 Agent 会正常继续运行。

### `pre_llm_call` 上下文注入 {#pre_llm_call-context-injection}

这是唯一一个返回值有意义的钩子。当 `pre_llm_call` 回调返回一个带有 `"context"` 键的字典（或纯字符串）时，Hermes 会将该文本注入到**当前轮次的用户消息**中。这是内存插件、RAG 集成、护栏以及任何需要为模型提供额外上下文的插件所使用的机制。
...
注入的上下文被附加到**用户消息**中，而不是系统提示词中。这是一个深思熟虑的设计选择：

- **提示词缓存保留** —— 系统提示词在各轮次间保持不变。Anthropic 和 OpenRouter 会缓存系统提示词前缀，因此保持其稳定可以在多轮对话中节省 75% 以上的输入 Token。如果插件修改了系统提示词，每一轮都会导致缓存失效。
- **瞬态** —— 注入仅在 API 调用时发生。对话历史记录中的原始用户消息永远不会被修改，也不会有任何内容持久化到会话数据库中。
- **系统提示词是 Hermes 的领地** —— 它包含模型特定的指导、工具强制规则、个性化指令和缓存的技能内容。插件是在用户输入之外贡献上下文，而不是通过更改 Agent 的核心指令。
#### 示例：记忆回溯插件
...
#### 示例：防护栏（Guardrails）插件

```python
"""防护栏插件 — 强制执行内容策略。"""

POLICY = """你必须在此会话中遵守以下内容策略：
- 严禁生成访问工作目录之外文件系统的代码
- 在执行破坏性操作前务必发出警告
- 拒绝涉及提取个人数据的请求"""
...
当多个插件从 `pre_llm_call` 返回上下文时，它们的输出会通过双换行符连接，并一并附加到用户消息中。顺序遵循插件发现顺序（按插件目录名称的字母顺序排列）。
...
**记忆提供程序插件**则使用基于约定的方法：在插件的 `cli.py` 文件中添加一个 `register_cli(subparser)` 函数。记忆插件发现系统会自动找到它，无需调用 `ctx.register_cli_command()`。详情请参阅 [记忆提供程序插件指南](/developer-guide/memory-provider-plugin#adding-cli-commands)。

**活动提供程序门控：** 记忆插件的 CLI 命令仅在配置中将其提供程序设为活动的 `memory.provider` 时才会显示。如果用户尚未设置你的提供程序，你的 CLI 命令就不会干扰帮助输出。

:::tip
本指南涵盖了**通用插件**（工具、钩子、CLI 命令）。有关专业插件类型，请参阅：
- [记忆提供程序插件](/developer-guide/memory-provider-plugin) — 跨会话知识后端
- [上下文引擎插件](/developer-guide/context-engine-plugin) — 替代的上下文管理策略
:::

### 通过 pip 分发

若要公开共享插件，请在你的 Python 包中添加一个入口点（entry point）：
