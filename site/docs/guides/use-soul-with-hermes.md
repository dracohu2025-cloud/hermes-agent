...
一个好的规则是：
- 如果它应该在任何地方都适用，就放在 `SOUL.md` 里
- 如果它只属于一个项目，就放在 `AGENTS.md` 里
...
当 Hermes 启动一个会话时，它会从 `HERMES_HOME` 读取 `SOUL.md`，扫描其中是否有提示词注入模式，必要时进行截断，并将其用作**智能体身份**——系统提示词中的第一个位置。这意味着 SOUL.md 完全取代了内置的默认身份文本。
