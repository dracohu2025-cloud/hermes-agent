...
# 用 Cron 自动化一切 {#automate-anything-with-cron}

[每日简报机器人教程](/guides/daily-briefing-bot) 涵盖了基础知识。本指南更进一步 —— 介绍五种你可以适配到自己工作流中的真实自动化模式。

完整的功能参考，请查阅 [定时任务 (Cron)](/user-guide/features/cron)。

:::info 核心概念
Cron 任务在全新的 Agent 会话中运行，不记得你当前的聊天记录。提示词必须是**完全自包含的** —— 包含 Agent 需要知道的一切信息。
:::

---

## 模式 1：网站变更监控 {#pattern-1-website-change-monitor}
<a id="key-concept"></a>

监控一个 URL 的变更，仅在内容发生变化时收到通知。

这里的 `script` 参数是秘密武器。一个 Python 脚本在每次执行前运行，其标准输出会成为 Agent 的上下文。脚本处理机械性工作（获取、对比）；Agent 处理推理（这个变化有趣吗？）。
...
:::tip [SILENT] 技巧
当 Agent 的最终响应包含 `[SILENT]` 时，消息传递会被抑制。这意味着你只在真正有事情发生时才会收到通知 —— 在安静时段不会有垃圾信息。
:::
...
:::warning 自包含的提示词
注意提示词中包含了确切的 `gh` 命令。Cron Agent 没有之前运行的记忆，也不知道你的偏好 —— 必须把所有东西都写清楚。
:::
...
脚本负责机械性的收集；Agent 则增加了推理层。

---

## 模式 5：多技能工作流 {#pattern-5-multi-skill-workflow}

将多个技能链接起来，完成复杂的定时任务。技能会在提示词执行前按顺序加载。
...
**让提示词自成一体。** 定时任务中的 Agent 没有你之前对话的记忆。请直接在提示词中包含 URL、仓库名称、格式偏好和交付说明。

**善用 `[SILENT]`。** 对于监控类任务，务必包含类似“如果没有任何变化，请回复 `[SILENT]`”的指令。这可以避免通知噪音。

**使用脚本进行数据收集。** `script` 参数允许 Python 脚本处理枯燥的部分（HTTP 请求、文件 I/O、状态跟踪）。Agent 只会看到脚本的 stdout，并对其应用推理。这比让 Agent 自己获取数据更便宜、更可靠。

**使用 `/cron run` 进行测试。** 在等待计划触发之前，使用 `/cron run &lt;job_id&gt;` 立即执行并验证输出是否正确。

**调度表达式。** 支持的格式：相对延迟（`30m`）、间隔（`every 2h`）、标准 cron 表达式（`0 9 * * *`）和 ISO 时间戳（`2025-06-15T09:00:00`）。不支持自然语言（如 `daily at 9am`）——请改用 `0 9 * * *`。

---

*完整的 cron 参考——所有参数、边界情况和内部原理——请参阅 [Scheduled Tasks (Cron)](/user-guide/features/cron)。*
