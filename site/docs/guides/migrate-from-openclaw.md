...
| Telegram | `credentials/telegram-default-allowFrom.json` | `TELEGRAM_ALLOWED_USERS` | 从 `allowFrom[]` 数组连接为逗号分隔 |
...
3.  **配置中的 env 子对象** — `openclaw.json` → `"env"` 或 `"env"."vars"`（有些设置将密钥存储在此处，而非单独的 `.env` 文件）
4.  **认证配置文件** — `~/.openclaw/agents/main/agent/auth-profiles.json`（每个 Agent 的凭证）
...
5.  **测试消息收发** — 如果你迁移了平台令牌，请重启网关：`systemctl --user restart hermes-gateway`
