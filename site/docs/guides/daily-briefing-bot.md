...
-   **消息传递配置**（可选但推荐） — 已设置好[Telegram](/docs/user-guide/messaging/telegram) 或 Discord 以及一个主频道
...
每个子代理独立并行搜索，然后主代理将所有内容合并成一份精美的简报。有关其工作原理的更多信息，请参阅[委托文档](/docs/user-guide/features/delegation)。
...
如果你启用了[记忆](/docs/user-guide/features/memory)，你可以存储跨会话持久化的偏好。但请记住——cron 任务在没有对话记忆的新会话中运行。要添加个人上下文，请直接将其融入提示中：
...
-   **[计划任务 (Cron)](/docs/user-guide/features/cron)** — 关于时间表格式、重复限制和投递选项的完整参考
-   **[委托](/docs/user-guide/features/delegation)** — 深入探讨并行子代理工作流
