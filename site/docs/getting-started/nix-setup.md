---
sidebar_position: 3
title: "Nix & NixOS 环境下安装"
description: "用 Nix 安装和部署 Hermes Agent — 从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 环境下安装 {#nix-nixos-setup}
...
:::info 与标准安装有何不同
`curl | bash` 安装器自行管理 Python、Node 和依赖项。Nix flake 取代了所有这些 — 每个 Python 依赖项都是由 [uv2nix](https://github.com/pyproject-nix/uv2nix) 构建的 Nix 派生项，运行时工具（Node.js、git、ripgrep、ffmpeg）已封装到二进制文件的 PATH 中。没有运行时 pip，没有 venv 激活，没有 `npm install`。
...
- **启用 flake 的 Nix** — 推荐使用 [Determinate Nix](https://install.determinate.systems)（默认启用 flakes）
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 会出现在您的 PATH 中。从这里开始，工作流与[标准安装](./installation.md)完全相同 — `hermes setup` 引导您选择提供商，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置位于 `~/.hermes/`。
...
:::warning 密钥是必需的
上述的 `environmentFiles` 行假设你已经配置了 [sops-nix](https://github.com/Mic92/sops-nix) 或 [agenix](https://github.com/ryantm/agenix)。该文件应至少包含一个 LLM 供应商密钥（例如 `OPENROUTER_API_KEY=sk-or-...`）。完整配置请参见[密钥管理](#secrets-management)。如果你还没有密钥管理器，可以先使用普通文件作为起点——只需确保它不能被 world 读取：
...
将 `addToSystemPackages = true` 会做两件事：将 `hermes` CLI 添加到系统 PATH 中，**并且**在系统范围内设置 `HERMES_HOME`，使得交互式 CLI 与网关服务共享状态（会话、技能、cron）。如果不设置，在 shell 中运行 `hermes` 会创建独立的 `~/.hermes/` 目录。
:::

<a id="container-aware-cli"></a>
:::info 容器感知的 CLI
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**每个** `hermes` 命令都会自动路由到受管理的容器中。这意味着你的交互式 CLI 会话与网关服务运行在相同的环境中——可以访问容器中安装的所有包和工具。

- 路由是透明的：`hermes chat`、`hermes sessions list`、`hermes version` 等都会在底层通过 exec 进入容器执行
- 所有 CLI 标志都会原样转发
- 如果容器未运行，CLI 会短暂重试（交互使用情况下 5 秒，带转圈动画；脚本情况下静默 10 秒），然后失败并显示明确的错误信息——不会静默降级
- 对于在 hermes 代码库上工作的开发者，设置 `HERMES_DEV=1` 可以绕过容器路由，直接运行本地代码
...
:::tip 发现可用的配置键
运行 `nix build .#configKeys && cat result` 可以查看从 Python 的 `DEFAULT_CONFIG` 中提取出的所有叶子配置键。你可以将现有的 `config.yaml` 粘贴到 `settings` 属性集中——两者的结构是一一对应的。
...
### 逃生舱：使用你自己的配置 {#escape-hatch-bring-your-own-config}
...
Nix 表达式中的值最终会进入 `/nix/store`，该目录全局可读。请始终使用 `environmentFiles` 配合密钥管理器。
:::
...
### OAuth / Auth 预置 {#oauth-auth-seeding}
...
- 你放在此处的任何其他文件，Agent 都会将其视为工作区文件。
...
### 带 OAuth 的 HTTP 传输 {#http-transport-with-oauth}
...
### 采样（服务端发起的 LLM 请求） {#sampling-server-initiated-llm-requests}
...
当 hermes 通过 NixOS 模块运行时，以下 CLI 命令会被**阻止**，并显示一条描述性错误信息，引导你前往 `configuration.nix`：
...
### 什么情况下会保留什么 {#what-persists-across-what}
...
只有当容器的**身份哈希**发生变化时，容器才会被重建。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 以及入口点脚本。环境变量、设置、文档或 hermes 包本身的变更**不会**触发重建。

:::warning 可写层丢失
当身份哈希发生变化时（镜像升级、新卷、新容器选项），容器会被销毁并从 `container.image` 重新拉取创建。可写层中通过 `apt install`、`pip install` 或 `npm install` 安装的包都会丢失。`/data` 和 `/home/hermes` 中的状态会保留（这些是 bind mount）。
...
### 目录插件（`extraPlugins`） {#directory-plugins-extraplugins}
...
### 入口点插件（`extraPythonPackages`） {#entry-point-plugins-extrapythonpackages}
...
| `no container with name or ID "hermes-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密码 sudo（参见[容器模式](#container-mode)章节） |
