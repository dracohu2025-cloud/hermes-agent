...
- 使用 Termux 的 `pkg` 来安装系统依赖（`git`、`python`、`nodejs`、`ripgrep`、`ffmpeg`、构建工具）
...
:::tip Termux 用户
`.[all]` 目前在 Android 上不可用，因为 `voice` 额外包会拉取 `faster-whisper`，而它依赖的 `ctranslate2` wheel 没有为 Android 发布。请使用 `.[termux]` 作为经过测试的移动安装路径，然后根据需要仅添加个别额外包。
:::
