...
sidebar_position: 9
title: "Git 工作树"
description: "使用 git 工作树和隔离的检出，在同一仓库上安全地运行多个 Hermes Agent"
---
...
Git **工作树**是为每个 Agent 提供其专属检出而不复制整个仓库的最安全方式。
...
对于并行 Agent，打开多个终端并在每个终端中运行 `hermes -w`——每次调用都会自动获得其专属的工作树和分支。
