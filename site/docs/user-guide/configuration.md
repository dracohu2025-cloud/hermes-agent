...
:::tip
备用模型配置**仅**通过 `config.yaml` 进行 — 没有对应的环境变量。关于触发条件、支持的提供商以及它与辅助任务和委托的交互的完整细节，请参阅[备用提供商](/docs/user-guide/features/fallback-providers)。
:::
...
可用提供商：`auto`、`openrouter`、`nous`、`codex`、`copilot`、`anthropic`、`main`、`zai`、`kimi-coding`、`minimax`，以及任何在[提供商注册表](/docs/reference/environment-variables)中注册的提供商。
...
在执行破坏性文件操作前自动创建文件系统快照。详情请参阅[检查点功能页面](/docs/user-guide/features/checkpoints)。
