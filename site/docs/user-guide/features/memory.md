...
Hermes Agent 拥有有限且经过筛选的记忆，这些记忆会在会话之间持久保存。这使得它能够记住你的偏好、你的项目、你的环境以及它学到的东西。
...
两者都存储在 `~/.hermes/memories/` 目录下，并在会话开始时作为冻结的快照注入到系统提示词中。智能体通过 `memory` 工具管理自己的记忆 — 它可以添加、替换或删除条目。

:::info
字符限制使记忆保持聚焦。当记忆已满时，智能体会合并或替换条目，为新信息腾出空间。
:::
...
格式包括：
- 一个标题，显示是哪个存储区（MEMORY 或 USER PROFILE）
- 使用百分比和字符计数，以便智能体了解容量
- 各个条目由 `§`（节符号）分隔符分隔
- 条目可以是多行的

**冻结快照模式：** 系统提示词注入在会话开始时捕获一次，会话中途永不更改。这是有意为之 — 它保留了 LLM 的前缀缓存以提高性能。当智能体在会话期间添加/删除记忆条目时，更改会立即持久化到磁盘，但直到下一个会话开始时才会出现在系统提示词中。工具响应始终显示实时状态。
...
没有 `read` 操作 — 记忆内容在会话开始时自动注入到系统提示词中。智能体将其记忆视为其对话上下文的一部分。

### 子字符串匹配

`replace` 和 `remove` 操作使用简短、唯一的子字符串匹配 — 你不需要完整的条目文本。`old_text` 参数只需要是一个能唯一标识一个条目的子字符串：
...
**最佳实践：** 当记忆使用率超过 80%（在系统提示词标题中可见）时，在添加新条目之前先合并条目。例如，将三个独立的“项目使用 X”条目合并为一个全面的项目描述条目。
...
记忆条目在被接受之前会进行注入和窃取模式的扫描，因为它们会被注入到系统提示词中。内容匹配威胁模式（提示词注入、凭据窃取、SSH 后门）或包含不可见 Unicode 字符的条目会被阻止。
...
| **管理** | 由智能体手动筛选 | 自动 — 所有会话已存储 |
| **Token 成本** | 每个会话固定（约 1,300 tokens） | 按需（需要时搜索） |

**记忆** 用于应始终在上下文中的关键事实。**会话搜索** 用于“我们上周讨论过 X 吗？”这类查询，智能体需要回忆过去对话的具体内容。
...
为了获得更深层次、AI 生成的、跨会话和跨平台的用户理解，你可以启用 [Honcho Memory](./honcho.md)。Honcho 以 `hybrid` 模式（默认）与内置记忆一起运行 — `MEMORY.md` 和 `USER.md` 保持不变，Honcho 在其之上添加一个持久的用户建模层。

```bash
hermes honcho setup
```

完整的配置、工具和 CLI 参考，请参阅 [Honcho Memory](./honcho.md) 文档。
