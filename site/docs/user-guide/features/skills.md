...
当遇到缺失的值时，Hermes 仅在技能实际在本地 CLI 中加载时才会安全地询问它。你可以跳过设置并继续使用该技能。消息平台永远不会在聊天中询问秘密 —— 它们会告诉你在本地使用 `hermes setup` 或 `~/.hermes/.env` 来设置。
...
技能也可以声明存储在 `config.yaml` 中的非秘密配置设置（路径、偏好）：
...
- **只读**：外部目录仅用于技能发现扫描。当 Agent 创建或编辑技能时，总是写入 `~/.hermes/skills/`。
- **本地优先**：如果同一个技能名称同时存在于本地目录和外部目录，则本地版本优先。
- **完全集成**：外部技能会出现在系统提示索引、`skills_list`、`skill_view` 中，并作为 `/skill-name` 斜杠命令使用——与本地技能没有区别。
- **不存在的路径会被静默跳过**：如果配置的目录不存在，Hermes 会忽略它而不报错。这对于可能并非每台机器都存在的可选共享目录很有用。
...
Agent 可以通过 `skill_manage` 工具创建、更新和删除自己的技能。这是 Agent 的**程序性记忆**——当它弄清楚一个非平凡的工作流程时，会将方法保存为技能以供将来重用。
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | 可通过 `hermes skills search &lt;query&gt; --source skills-sh` 搜索。当 skills.sh 的别名与仓库文件夹不同时，Hermes 会解析别名风格的技能。 |
...
这是基于 URL 的发现方式，来自发布 `/.well-known/skills/index.json` 的网站。它不是一个单一的集中式中心——它是一种网络发现约定。
...
所有通过 hub 安装的技能都会经过一个**安全扫描器**的检查，它会检测数据外泄、提示词注入、破坏性命令、供应链信号以及其他威胁。
...
当你已经审查过某个第三方技能，并希望覆盖一个非危险性的策略阻止时，可以使用 `--force`：
...
- `--force` **不能**覆盖 `dangerous`（危险）的扫描判定。
- 官方的可选技能（`official/...`）被视为内置信任，不会显示第三方警告面板。
...
Hub 现在会追踪足够的来源信息，以便重新检查已安装技能的上游副本：
...
这利用了存储的源标识符加上当前上游捆绑包内容的哈希值来检测变更。

:::tip GitHub 速率限制
Skills hub 操作使用 GitHub API，对于未认证用户，其速率限制为每小时 60 次请求。如果在安装或搜索时遇到速率限制错误，请在 `.env` 文件中设置 `GITHUB_TOKEN`，以将限制提高到每小时 5,000 次请求。发生此情况时，错误信息会包含可操作的提示。
:::
