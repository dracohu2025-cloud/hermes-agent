...
| **[插件钩子](#plugin-hooks)** | 在[插件](/docs/user-guide/features/plugins)中使用 `ctx.register_hook()` | CLI + 网关 | 工具拦截、指标收集、防护规则 |
...
[插件](/docs/user-guide/features/plugins)可以注册在 **CLI 和网关**会话中都触发的钩子。这些钩子通过插件 `register()` 函数中的 `ctx.register_hook()` 以编程方式注册。
...
有关创建插件的完整细节，请参阅 **[插件指南](/docs/user-guide/features/plugins)**。
