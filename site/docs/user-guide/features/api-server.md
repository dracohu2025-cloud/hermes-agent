...
你的 Agent 会使用其完整的工具集（终端、文件操作、网络搜索、记忆、技能）处理请求，并返回最终响应。在流式传输时，工具进度指示器会内联显示，以便前端可以展示 Agent 正在做什么。
...
**流式传输** (`"stream": true`)：返回服务器发送事件（SSE），包含逐词元的响应块。当在配置中启用流式传输时，词元会在 LLM 生成时实时发出。当禁用时，完整的响应会作为单个 SSE 块发送。

**流中的工具进度**：当 Agent 在流式请求期间调用工具时，在工具开始执行时，简短的进度指示器会被注入到内容流中（例如 `` `💻 pwd` ``, `` `🔍 Python docs` ``）。这些会作为内联 Markdown 出现在 Agent 的响应文本之前，为 Open WebUI 等前端提供工具执行的实时可见性。
...
服务器从存储的响应链中重建完整对话 —— 所有之前的工具调用和结果都被保留。
...
当前端发送 `system` 消息（Chat Completions）或 `instructions` 字段（Responses API）时，hermes-agent **会将其叠加在**其核心系统提示之上。你的 Agent 保留其所有工具、记忆和技能 —— 前端的系统提示添加额外的指令。
...
大多数已记录的前端（如 Open WebUI）是服务器到服务器连接，完全不需要 CORS。

## 兼容的前端

任何支持 OpenAI API 格式的前端都可以工作。已测试/记录集成的有：
...
- **响应存储** — 存储的响应（用于 `previous_response_id`）会持久化保存在 SQLite 中，并在网关重启后保留。最多存储 100 个响应（采用 LRU 淘汰策略）。
- **不支持文件上传** — 目前尚不支持通过 API 上传文件进行视觉/文档分析。
- **Model 字段仅作展示** — 请求中的 `model` 字段会被接受，但实际使用的 LLM 模型是在服务端的 config.yaml 中配置的。
