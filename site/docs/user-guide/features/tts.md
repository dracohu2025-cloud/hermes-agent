...
tts:
  ...
  speed: 1.0                    # 全局语速乘数（特定提供商的设置会覆盖此值）
  edge:
...
**语速控制**：默认情况下，全局的 `tts.speed` 值适用于所有提供商。每个提供商都可以用自己的 `speed` 设置覆盖它（例如 `tts.openai.speed: 1.5`）。特定提供商的语速设置优先于全局值。默认值为 `1.0`（正常语速）。
...
如果没有安装 ffmpeg，Edge TTS、MiniMax TTS 和 NeuTTS 的音频将作为常规音频文件发送（可以播放，但显示为矩形播放器而非语音气泡）。
...
**Groq API** — 需要 `GROQ_API_KEY`。当你需要一个免费的托管 STT 选项时，这是一个很好的云端备选方案。
