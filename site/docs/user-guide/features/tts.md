...
如果没有安装 ffmpeg，Edge TTS、MiniMax TTS 和 NeuTTS 的音频会作为普通音频文件发送（可以播放，但会显示为矩形播放器，而不是语音气泡）。
...
**Groq API** — 需要 `GROQ_API_KEY`。当你想要一个免费的托管 STT 选项时，这是一个很好的云端备选方案。
