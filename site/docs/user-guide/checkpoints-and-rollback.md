...
代理在**每个对话轮次（每个目录）最多创建一个检查点**，因此长时间运行的会话不会产生大量快照。
...
- 这些提交构成了一个检查点历史记录，你可以通过 `/rollback` 命令查看和恢复。
...
📸 检查点列表，针对 /path/to/project：
  ...
  1. 4270a8c  2026-03-16 04:36  patch 操作前  (1 个文件, +1/-0)
  2. eaf4c1f  2026-03-16 04:35  write_file 操作前
...
这会显示一个 git diff 统计摘要，然后是实际的差异内容：

```text
test.py | 2 +-
 1 个文件被更改，1 处插入(+)，1 处删除(-)
...
3. 恢复工作目录中受跟踪的文件。
4. **撤销最后一个对话轮次**，使代理的上下文与恢复后的文件系统状态匹配。

成功后：

```text
✅ 已恢复到检查点 4270a8c5: patch 操作前
已自动保存回滚前快照。
...
对话撤销确保代理不会“记住”已被回滚的更改，避免在下一轮次中产生混淆。
...
- **非致命错误** — 检查点管理器内的所有错误都记录在调试级别；你的工具会继续运行。
...
每个 `<hash>` 派生自工作目录的绝对路径。在每个影子仓库内，你会找到：

- 标准的 Git 内部结构（`HEAD`、`refs/`、`objects/`）
- 一个 `info/exclude` 文件，包含精心挑选的忽略列表
- 一个 `HERMES_WORKDIR` 文件，指向原始的项目根目录

通常你永远不需要手动操作这些内容。

## 最佳实践

- **保持检查点启用** — 它们默认开启，并且在未修改文件时没有任何开销。
- **恢复前使用 `/rollback diff`** — 预览将要更改的内容，以选择正确的检查点。
- **当只想撤销代理驱动的更改时，使用 `/rollback` 而非 `git reset`**。
- **与 Git worktrees 结合使用以获得最大安全性** — 让每个 Hermes 会话都在其自己的 worktree/分支中运行，检查点作为额外的一层保护。

关于在同一仓库上并行运行多个代理，请参阅 [Git worktrees](./git-worktrees.md) 指南。
