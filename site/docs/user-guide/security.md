...
:::tip
对于生产网关部署，请使用 `docker`、`modal` 或 `daytona` 后端，将 Agent 命令与你的主机系统隔离。这完全消除了对危险命令审批的需求。
:::
...
你可以限制 Agent 通过其网页和浏览器工具可以访问哪些网站。这对于防止 Agent 访问内部服务、管理面板或其他敏感 URL 很有用。
...
这可以将网关的消息传递连接与 Agent 的命令执行分离开来。
