...
:::info
以下内容是当此技能触发时 Hermes 加载的完整技能定义。这是技能激活时 Agent 看到的指令。
:::

<a id="arxiv-research"></a>
# arXiv Research

通过 arXiv 的免费 REST API 搜索并获取学术论文。不需要 API 密钥，不需要额外依赖——只需 curl。
...
<a id="searching-papers"></a>
## 搜索论文

API 返回 Atom XML 格式。可以用 `grep`/`sed` 解析，或通过 `python3` 管道处理以得到清晰输出。
...
<a id="clean-output-parse-xml-to-readable-format"></a>
### 清理输出（将 XML 解析为可读格式）
...
<a id="fetching-specific-papers"></a>
## 获取特定论文
...
如需处理本地 PDF，请参考 `ocr-and-documents` 技能。

<a id="common-categories"></a>
## 常见类别
...
`scripts/search_arxiv.py` 脚本负责 XML 解析并提供清晰的输出：
...
<a id="semantic-scholar-citations-related-papers-author-profiles"></a>
## Semantic Scholar（引文、相关论文、作者简介）

arXiv 不提供引文数据或推荐功能。请使用 **Semantic Scholar API** 来获取这些数据——免费，基础使用无需 API 密钥（每秒 1 次请求），返回 JSON 格式。

<a id="get-paper-details-citations"></a>
### 获取论文详情 + 引文
...
<a id="get-citations-of-a-paper-who-cited-it"></a>
### 获取某篇论文的引文列表（谁引用了它）
...
<a id="get-references-from-a-paper-what-it-cites"></a>
### 获取某篇论文的参考文献列表（它引用了哪些）

```bash
curl -s "https://api.semanticscholar.org/graph/v1/paper/arXiv:2402.03300/references?fields=title,authors,year,citationCount&limit=10" | python3 -m json.tool
```

<a id="search-papers-alternative-to-arxiv-search-returns-json"></a>
...
<a id="get-paper-recommendations"></a>
### 获取论文推荐
...
<a id="useful-semantic-scholar-fields"></a>
...
<a id="complete-research-workflow"></a>
## 完整研究工作流程
...
5. **寻找相关工作**：`curl -s "https://api.semanticscholar.org/graph/v1/paper/arXiv:ID/references?fields=title,citationCount&limit=20"`
...
| Semantic Scholar | 1 次请求 / 秒 | 无需（带 API 密钥可提升至 100 次/秒） |

<a id="notes"></a>
## 注意事项

- arXiv 返回 Atom XML 格式数据 —— 使用辅助脚本或解析代码片段获得整洁输出
- Semantic Scholar 返回 JSON 格式 —— 可通过 `python3 -m json.tool` 管道输出以提高可读性
- arXiv ID 格式：旧格式（`hep-th/0601001`）与新格式（`2402.03300`）
- PDF 链接：`https://arxiv.org/pdf/{id}` — 摘要链接：`https://arxiv.org/abs/{id}`
- HTML 版本（若有）：`https://arxiv.org/html/{id}`
- 如需本地处理 PDF，请参阅 `ocr-and-documents` 技能

<a id="id-versioning"></a>
## ID 版本管理

- `arxiv.org/abs/1706.03762` 始终解析到**最新**版本
- `arxiv.org/abs/1706.03762v1` 指向**特定**的不可变版本
- 生成引用时，请保留您实际阅读的版本后缀，以防止引用漂移（后续版本可能大幅修改内容）
- API 返回的 `&lt;id&gt;` 字段为带版本的 URL（例如 `http://arxiv.org/abs/1706.03762v7`）

<a id="withdrawn-papers"></a>
## 已撤稿论文

论文在提交后可能被撤稿。出现这种情况时：
- `&lt;summary&gt;` 字段会包含撤稿通知（查找 "withdrawn" 或 "retracted"）
- 元数据字段可能不完整
- 在将结果视为有效论文前，务必检查摘要内容
