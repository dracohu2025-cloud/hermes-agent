...
使用 [OpenCode](https://opencode.ai) 作为由 Hermes 终端/进程工具编排的自主编码工作器。OpenCode 是一个与提供商无关的开源 AI 编码 Agent，提供 TUI 和 CLI。

## 何时使用 {#when-to-use}

- 用户明确要求使用 OpenCode
- 你希望使用外部编码 Agent 来实现/重构/审查代码
- 你需要长时间运行的编码会话，并希望检查进度
...
## 二进制文件解析（重要） {#binary-resolution-important}

Shell 环境可能解析到不同的 OpenCode 二进制文件。如果终端和 Hermes 中的行为不一致，请检查：
...
```
**重要提示：** 请勿使用 `/exit`——它不是有效的 OpenCode 命令，反而会打开一个 Agent 选择对话框。请使用 Ctrl+C（`\x03`）或 `process(action="kill")` 来退出。
...
- `/exit` **不是**有效命令——它会打开一个 Agent 选择器。请使用 Ctrl+C 退出 TUI。
- PATH 不匹配可能导致选错 OpenCode 二进制文件/模型配置。
- 如果 OpenCode 卡住，在终止前先检查日志：
  - `process(action="log", session_id="&lt;id&gt;")`
- 避免多个 OpenCode 会话共享同一个工作目录。
- 在 TUI 中可能需要按两次回车才能提交（第一次完成文本输入，第二次发送）。
...
4. 对于长任务，通过 `process` 日志提供进度更新。
5. 报告具体结果（修改的文件、测试、剩余风险）。
