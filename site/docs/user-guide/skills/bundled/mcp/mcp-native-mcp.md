...
# Native MCP 客户端 {#native-mcp-client}

Hermes Agent 内置了一个 MCP 客户端，它在启动时连接到 MCP 服务器，发现其工具，并将它们作为一等工具提供给 Agent 直接调用。无需桥接 CLI——来自 MCP 服务器的工具会与 `terminal`、`read_file` 等内置工具一同出现。
...
### Stdio 传输（命令 + 参数） {#stdio-transport-command-args}
...
```
### HTTP 传输（url） {#http-transport-url}
...
子进程会继承一个**经过过滤**的环境（参见下方安全章节），以及你在 `env` 中指定的任何变量。

### HTTP / StreamableHTTP 传输 {#http-streamablehttp-transport}
...
### 环境变量过滤 {#environment-variable-filtering}

对于 stdio 服务器，Hermes **不会**将你的完整 shell 环境传递给 MCP 子进程。仅继承以下安全基线变量：
...
### 错误消息中的凭据擦除 {#credential-stripping-in-error-messages}
...
### "MCP server 'X' requires HTTP transport but mcp.client.streamable_http is not available" {#mcp-server-x-requires-http-transport-but-mcp-client-streamablehttp-is-not-available}
...
### 文件系统服务器 (npx) {#filesystem-server-npx}
...
### 带身份验证的 GitHub 服务器 {#github-server-with-authentication}
...
### 远程 HTTP 服务器 {#remote-http-server}
...
## 采样（服务器发起的 LLM 请求） {#sampling-server-initiated-llm-requests}
