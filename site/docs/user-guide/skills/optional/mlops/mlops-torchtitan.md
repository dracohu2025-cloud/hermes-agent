...
description: "提供基于 torchtitan 的 PyTorch 原生分布式 LLM 预训练，支持 4D 并行（FSDP2、TP、PP、CP）"
---

{/* 本页面由 skill 的 SKILL.md 通过 website/scripts/generate-skill-docs.py 自动生成。请编辑源文件 SKILL.md，不要编辑此页面。 */}

<a id="distributed-llm-pretraining-torchtitan"></a>
# 分布式 LLM 预训练 Torchtitan

提供基于 torchtitan 的 PyTorch 原生分布式 LLM 预训练，支持 4D 并行（FSDP2、TP、PP、CP）。适用于在 8 到 512+ GPU 规模下预训练 Llama 3.1、DeepSeek V3 或自定义模型，支持 Float8、torch.compile 和分布式检查点。
...
:::info
以下是 skill 的完整定义。当此 skill 被触发时，Hermes 会加载该定义。当 skill 激活时，这是 Agent 看到的指令。
:::

<a id="torchtitan-pytorch-native-distributed-llm-pretraining"></a>
...
<a id="workflow-1-pretrain-llama-3-1-8b-on-single-node"></a>
...
<a id="workflow-2-multi-node-training-with-slurm"></a>
...
<a id="workflow-3-enable-float8-training-for-h100s"></a>
...
<a id="workflow-4-4d-parallelism-for-405b-models"></a>
...
context_parallel_degree = 1      # 长序列的 CP

[training]
local_batch_size = 32
seq_len = 8192
```
**Step 3: 在 512 块 GPU 上运行**
...
**在以下场景使用 TorchTitan：**
- 从头预训练 LLM（8B 到 405B+）
- 需要 PyTorch 原生方案，无第三方依赖
- 需要可组合的 4D 并行（FSDP2、TP、PP、CP）
- 在 H100 上用 Float8 支持进行训练
...
<a id="performance-benchmarks-h100"></a>
## 性能基准测试 (H100)

| 模型 | GPU数 | 并行方式 | TPS/GPU | 技术 |
|------|-------|----------|---------|------|
...
**FSDP2 配置**：参见 [references/fsdp.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/torchtitan/references/fsdp.md)，了解 FSDP2 与 FSDP1 的详细对比及其对应的 ZeRO 级别。

**Float8 训练**：参见 [references/float8.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/torchtitan/references/float8.md)，了解张量级缩放与行级缩放方案。
