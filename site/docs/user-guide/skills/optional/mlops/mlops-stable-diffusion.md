...
通过 HuggingFace Diffusers 使用 Stable Diffusion 模型实现最先进的文本到图像生成。当需要从文本提示生成图像、执行图到图转换、修复图像，或构建自定义扩散管道时使用此功能。
...
使用 HuggingFace Diffusers 库生成 Stable Diffusion 图像的全面指南。

<a id="when-to-use-stable-diffusion"></a>
## 何时使用 Stable Diffusion

**在以下场景使用 Stable Diffusion：**
- 根据文本描述生成图像
- 执行图到图转换（风格迁移、增强）
- 修复图像（填充遮罩区域）
- 外延图像（扩展到边界之外）
...
pip install xformers  # 可选：节省内存的注意力机制
```

<a id="basic-text-to-image"></a>
### 基础文本到图像
...
<a id="using-sdxl-higher-quality"></a>
### 使用 SDXL（更高质量）
...
<a id="pipeline-inference-flow"></a>
### Pipeline 推理流程
...
mask = Image.open("mask.png")  # 白色区域为修补区域

result = pipe(
    prompt="A red car parked on the street",  # "一辆停在街上的红色汽车"
    image=image,
    mask_image=mask,
    num_inference_steps=50
).images[0]
...
image = pipe(
    prompt="A beautiful house in the style of Van Gogh",  # "梵高风格的漂亮房子"
    image=control_image,
    num_inference_steps=30
).images[0]
```

<a id="available-controlnets"></a>
### 可用的 ControlNet
...
<a id="xformers-memory-efficient-attention"></a>
### xFormers 高效注意力
...
<a id="vae-slicing-for-large-images"></a>
### VAE 切片以支持大图像
```python
# 对大图按瓦片解码潜变量
...
<a id="loading-different-precisions"></a>
### 加载不同精度
...
<a id="loading-specific-components"></a>
### 加载特定组件
...
<a id="workflow-1-high-quality-generation"></a>
### 工作流 1：高质量生成
...
<a id="workflow-2-fast-prototyping"></a>
### 工作流 2：快速原型验证
