---
title: "Docker 管理"
sidebar_label: "Docker 管理"
description: "管理 Docker 容器、镜像、卷、网络和 Compose 堆栈——生命周期操作、调试、清理和 Dockerfile 优化"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

<a id="docker-management"></a>
# Docker 管理

管理 Docker 容器、镜像、卷、网络和 Compose 堆栈——生命周期操作、调试、清理和 Dockerfile 优化。

<a id="skill-metadata"></a>
## 技能元数据

|          |                                                                                      |
| -------- | ------------------------------------------------------------------------------------ |
| 来源     | 可选 — 通过 `hermes skills install official/devops/docker-management` 安装           |
| 路径     | `optional-skills/devops/docker-management`                                          |
| 版本     | `1.0.0`                                                                              |
| 作者     | sprmn24                                                                              |
| 许可证   | MIT                                                                                  |
| 平台     | linux, macos, windows                                                                |
| 标签     | `docker`, `containers`, `devops`, `infrastructure`, `compose`, `images`, `volumes`, `networks`, `debugging` |

<a id="reference-full-skill-md"></a>
## 参考：完整的 SKILL.md

:::info
以下是 Hermes 在该技能被触发时加载的完整技能定义。这就是 Agent 在技能激活时看到的指令。
:::

# Docker 管理

使用标准 Docker CLI 命令管理 Docker 容器、镜像、卷、网络和 Compose 堆栈。除了 Docker 本身之外，无需其他依赖。

<a id="when-to-use"></a>
## 何时使用

- 运行、停止、重启、移除或检查容器
- 构建、拉取、推送、标记或清理 Docker 镜像
- 使用 Docker Compose（多服务堆栈）
- 管理卷或网络
- 调试崩溃的容器或分析日志
- 检查 Docker 磁盘使用情况或释放空间
- 审查或优化 Dockerfile

<a id="prerequisites"></a>
## 前置条件

- Docker Engine 已安装并运行
- 用户已添加到 `docker` 组（或使用 `sudo`）
- Docker Compose v2（现代 Docker 安装中已包含）

快速检查：

```bash
docker --version && docker compose version
```

<a id="quick-reference"></a>
## 快速参考

| 任务                              | 命令                                              |
| --------------------------------- | ------------------------------------------------- |
| 运行容器（后台）                  | `docker run -d --name NAME IMAGE`                 |
| 停止 + 移除                       | `docker stop NAME && docker rm NAME`              |
| 查看日志（持续追踪）              | `docker logs --tail 50 -f NAME`                   |
| 进入容器 Shell                    | `docker exec -it NAME /bin/sh`                    |
| 列出所有容器                      | `docker ps -a`                                    |
| 构建镜像                          | `docker build -t TAG .`                           |
| Compose 启动                      | `docker compose up -d`                            |
| Compose 停止                      | `docker compose down`                             |
| 磁盘使用情况                      | `docker system df`                                |
| 清理悬挂资源                      | `docker image prune && docker container prune`    |

<a id="procedure"></a>
## 操作步骤

<a id="1-identify-the-domain"></a>
### 1. 识别领域

判断请求属于哪个领域：

- **容器生命周期** → run, stop, start, restart, rm, pause/unpause
- **容器交互** → exec, cp, logs, inspect, stats
- **镜像管理** → build, pull, push, tag, rmi, save/load
- **Docker Compose** → up, down, ps, logs, exec, build, config
- **卷与网络** → create, inspect, rm, prune, connect
- **故障排查** → 日志分析、退出码、资源问题

<a id="2-container-operations"></a>
### 2. 容器操作

**运行新容器：**

```bash
# 后台服务，带端口映射
docker run -d --name web -p 8080:80 nginx

# 带环境变量
docker run -d -e POSTGRES_PASSWORD=secret -e POSTGRES_DB=mydb --name db postgres:16

# 持久化数据（命名卷）
docker run -d -v pgdata:/var/lib/postgresql/data --name db postgres:16

# 开发环境（绑定挂载源码）
docker run -d -v $(pwd)/src:/app/src -p 3000:3000 --name dev my-app

# 交互式调试（退出后自动删除）
docker run -it --rm ubuntu:22.04 /bin/bash

# 带资源限制和重启策略
docker run -d --memory=512m --cpus=1.5 --restart=unless-stopped --name app my-app
```
关键标志：`-d` 后台运行，`-it` 交互式+终端，`--rm` 自动删除，`-p` 端口映射（主机端口:容器端口），`-e` 环境变量，`-v` 数据卷，`--name` 容器名，`--restart` 重启策略。

**管理运行中的容器：**

```bash
docker ps                        # 列出运行中的容器
docker ps -a                     # 列出所有容器（包括已停止的）
docker stop NAME                 # 优雅停止容器
docker start NAME                # 启动已停止的容器
docker restart NAME              # 重启容器（停止+启动）
docker rm NAME                   # 删除已停止的容器
docker rm -f NAME                # 强制删除运行中的容器
docker container prune           # 删除所有已停止的容器
```

**与容器交互：**

```bash
docker exec -it NAME /bin/sh          # 进入Shell（如果可用，使用/bin/bash）
docker exec NAME env                   # 查看环境变量
docker exec -u root NAME apt update    # 以特定用户身份运行命令
docker logs --tail 100 -f NAME         # 实时查看最后100行日志
docker logs --since 2h NAME            # 查看最近2小时的日志
docker cp NAME:/path/file ./local      # 从容器复制文件到本地
docker cp ./file NAME:/path/           # 从本地复制文件到容器
docker inspect NAME                    # 查看容器完整详细信息（JSON格式）
docker stats --no-stream               # 查看资源使用快照
docker top NAME                        # 查看容器中运行的进程
```

<a id="3-image-management"></a>
### 3. 镜像管理

```bash
# 构建
docker build -t my-app:latest .
docker build -t my-app:prod -f Dockerfile.prod .
docker build --no-cache -t my-app .              # 干净重建（不使用缓存）
DOCKER_BUILDKIT=1 docker build -t my-app .       # 使用BuildKit加速构建

# 拉取与推送
docker pull node:20-alpine
docker login ghcr.io
docker tag my-app:latest registry/my-app:v1.0
docker push registry/my-app:v1.0

# 查看
docker images                          # 列出本地镜像
docker history IMAGE                   # 查看镜像分层
docker inspect IMAGE                   # 查看完整详细信息

# 清理
docker image prune                     # 移除悬空（未标记）镜像
docker image prune -a                  # 移除所有未使用的镜像（谨慎操作！）
docker image prune -a --filter "until=168h"   # 移除超过7天的未使用镜像
```

<a id="4-docker-compose"></a>
### 4. Docker Compose

```bash
# 启动/停止
docker compose up -d                   # 后台启动所有服务
docker compose up -d --build           # 启动前重新构建镜像
docker compose down                    # 停止并删除容器
docker compose down -v                 # 同时删除数据卷（会销毁数据！）

# 监控
docker compose ps                      # 列出所有服务
docker compose logs -f api             # 实时查看特定服务的日志
docker compose logs --tail 50          # 查看所有服务最后50行日志

# 交互
docker compose exec api /bin/sh        # 进入运行中服务的Shell
docker compose run --rm api npm test   # 运行一次性命令（创建新容器）
docker compose restart api             # 重启特定服务

# 验证
docker compose config                  # 验证并查看解析后的配置
```
**最小的 `compose.yml` 示例：**

```yaml
services:
  api:
    build: .
    ports:
      - "3000:3000"
    environment:
      - DATABASE_URL=postgres://user:pass@db:5432/mydb
    depends_on:
      db:
        condition: service_healthy

  db:
    image: postgres:16-alpine
    environment:
      POSTGRES_USER: user
      POSTGRES_PASSWORD: pass
      POSTGRES_DB: mydb
    volumes:
      - pgdata:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U user"]
      interval: 10s
      timeout: 5s
      retries: 5

volumes:
  pgdata:
```

<a id="5-volumes-and-networks"></a>
### 5. 数据卷与网络

```bash
# 数据卷
docker volume ls                       # 列出数据卷
docker volume create mydata            # 创建命名数据卷
docker volume inspect mydata           # 查看详情（挂载点等）
docker volume rm mydata                # 删除（如果正在使用则失败）
docker volume prune                    # 删除未使用的数据卷

# 网络
docker network ls                      # 列出网络
docker network create mynet            # 创建桥接网络
docker network inspect mynet           # 查看详情（已连接的容器）
docker network connect mynet NAME      # 将容器连接到网络
docker network disconnect mynet NAME   # 断开容器与网络
docker network rm mynet                # 删除网络
docker network prune                   # 删除未使用的网络
```

<a id="6-disk-usage-and-cleanup"></a>
### 6. 磁盘使用情况与清理

在清理之前，请始终先进行诊断：

```bash
# 检查什么占用了空间
docker system df                       # 概要
docker system df -v                    # 详细分解

# 针对性清理（安全）
docker container prune                 # 停止的容器
docker image prune                     # 悬挂镜像
docker volume prune                    # 未使用的数据卷
docker network prune                   # 未使用的网络

# 激进清理（务必先与用户确认！）
docker system prune                    # 容器 + 镜像 + 网络
docker system prune -a                 # 还包括未使用的镜像
docker system prune -a --volumes       # 全部清理——命名数据卷也包含在内
```

**警告：** 在没有与用户确认的情况下，切勿运行 `docker system prune -a --volumes`。这会删除可能包含重要数据的命名数据卷。

<a id="pitfalls"></a>
## 常见陷阱

| 问题 | 原因 | 解决方法 |
|------|------|----------|
| 容器立即退出 | 主进程完成或崩溃 | 检查 `docker logs NAME`，尝试 `docker run -it --entrypoint /bin/sh IMAGE` |
| “端口已被分配” | 另一进程正在使用该端口 | 用 `docker ps` 或 `lsof -i :PORT` 找到它 |
| “设备上没有多余空间” | Docker 磁盘已满 | 执行 `docker system df` 然后针对性清理 |
| 无法连接到容器 | 应用在容器内部绑定到了 127.0.0.1 | 应用必须绑定到 `0.0.0.0`，检查 `-p` 映射 |
| 数据卷权限被拒绝 | 主机与容器的 UID/GID 不匹配 | 使用 `--user $(id -u):$(id -g)` 或修复权限 |
| Compose 服务之间无法互相访问 | 错误的网络或服务名 | 服务使用服务名作为主机名，检查 `docker compose config` |
| 构建缓存不生效 | 构建缓存不生效 | Dockerfile 中的层顺序错误 |
| 镜像太大 | 没有多阶段构建，没有 .dockerignore | 使用多阶段构建，添加 `.dockerignore` |
<a id="verification"></a>
## 验证

执行任何 Docker 操作后，请验证结果：

- **容器已启动？** → `docker ps`（检查状态是否为 "Up"）
- **日志正常？** → `docker logs --tail 20 NAME`（无错误信息）
- **端口可访问？** → `curl -s http://localhost:PORT` 或 `docker port NAME`
- **镜像已构建？** → `docker images | grep TAG`
- **Compose 堆栈健康？** → `docker compose ps`（所有服务为 "running" 或 "healthy"）
- **磁盘空间释放？** → `docker system df`（对比操作前后的情况）

<a id="dockerfile-optimization-tips"></a>
## Dockerfile 优化建议

在审查或创建 Dockerfile 时，可建议以下改进：

1. **多阶段构建** —— 将构建环境与运行环境分离，以减小最终镜像大小
2. **分层排序** —— 将依赖项放在源代码之前，避免代码变更使缓存层失效
3. **合并 RUN 命令** —— 减少层数，镜像更小
4. **使用 .dockerignore** —— 排除 `node_modules`、`.git`、`__pycache__` 等
5. **固定基础镜像版本** —— 使用 `node:20-alpine` 而非 `node:latest`
6. **以非 root 用户运行** —— 添加 `USER` 指令以增强安全性
7. **使用精简版或 Alpine 基础镜像** —— 使用 `python:3.12-slim` 而非 `python:3.12`
