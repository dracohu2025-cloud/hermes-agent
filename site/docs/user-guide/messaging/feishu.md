...
- `websocket` — 推荐；Hermes 发起出站连接，你无需提供公共 webhook 端点
...
### 推荐方式：扫码创建（一条命令） {#recommended-scan-to-create-one-command}
...
### 备选方式：手动设置 {#alternative-manual-setup}
...
### 推荐方式：WebSocket 模式 {#recommended-websocket-mode}
...
### 可选方式：Webhook 模式 {#optional-webhook-mode}
...
该令牌同样可在飞书应用的**事件订阅**部分找到。设置后，每个入站 Webhook 负载的 `header` 对象中必须包含匹配的 `token`。令牌不匹配的请求将被拒绝并返回 HTTP 401。
...
如需按聊天进行控制，可在 `group_rules` 条目中设置 `require_mention` — 请参阅下面的[按群组访问控制](#per-group-access-control)。
...
这也是**命令审批**的工作方式 — 当 Agent 需要执行危险命令时，它会发送一个带有“允许一次”/“会话”/“始终”/“拒绝”按钮的交互式卡片。用户点击按钮后，卡片操作回调会将审批决定传回给 Agent。

### 必需的飞书应用配置 {#required-feishu-app-configuration}

交互式卡片需要在飞书开发者控制台中进行**三个**配置步骤。缺少任何一个都会导致用户在点击卡片按钮时出现错误 **200340**。
1. **订阅卡片动作事件：**  
   在 **事件订阅** 中，将 `card.action.trigger` 添加到已订阅事件中。

2. **启用交互式卡片能力：**  
   在 **应用功能 > 机器人** 中，确保 **交互式卡片** 开关已开启。这告诉飞书你的应用可以接收卡片动作回调。

3. **配置卡片请求 URL（仅限 webhook 模式）：**  
   在 **应用功能 > 机器人 > 消息卡片请求地址** 中，将 URL 设置为你事件 webhook 的同一端点（例如 `https://your-server:8765/feishu/webhook`）。在 WebSocket 模式下，SDK 会自动处理此配置。

:::warning
如果没有完成以上三个步骤，飞书可以成功*发送*交互式卡片（发送只需 `im:message:send` 权限），但点击任何按钮都会返回错误 200340。卡片看似正常——只有用户与它交互时才会暴露错误。
:::
...
- **`pairing`** —— 静态列表 ∪ 运行时可批准的存储。适用于版主可以实时授予权限的发布场景。
...
- 授予 `docs:doc:readonly` 和 `drive:drive:readonly` 作用域，以便处理器能够读取文档内容。
...
## Markdown 渲染与 Post 回退 {#markdown-rendering-and-post-fallback}
...
## 处理状态反应 {#processing-status-reactions}
...
WebSocket 和每个群组的 ACL 设置通过 `config.yaml` 中的 `platforms.feishu.extra` 配置（参见上面的 [WebSocket 调优](#websocket-tuning) 和 [每个群组的访问控制](#per-group-access-control)）。
...
| 点击审批按钮时出现错误 200340 | 在飞书开发者控制台中启用**交互卡片**能力并配置**卡片请求地址**。请参见上方的[飞书应用必需配置](#required-feishu-app-configuration)。 |
