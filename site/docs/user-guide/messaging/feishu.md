...
Hermes Agent 可以作为功能齐全的机器人集成到飞书和 Lark 中。连接后，你可以在私聊或群聊中与 Agent 对话，在家庭聊天中接收定时任务结果，并通过常规网关流程发送文本、图片、音频和文件附件。
...
如果你将白名单留空，任何能接触到机器人的用户都可能使用它。在群聊中，消息处理前会检查发送者的 open_id 是否在白名单中。
...
此令牌同样可在你的飞书应用的 **事件订阅** 部分找到。设置后，每个入站 webhook 负载的 `header` 对象中必须包含匹配的 `token`。不匹配的令牌将被拒绝，并返回 HTTP 401。
...
在所有模式下，消息处理前，机器人必须在群中被明确 @提及（或 @all）。私聊消息不受此限制。
...
**命令审批** 也是通过这种方式工作的 —— 当 Agent 需要运行危险命令时，它会发送一个带有“允许一次”/“允许会话”/“始终允许”/“拒绝”按钮的交互式卡片。用户点击按钮后，卡片操作回调将审批决定传回给 Agent。
...
:::warning
缺少以上三个步骤中的任何一步，飞书都能成功*发送*交互式卡片（发送仅需 `im:message:send` 权限），但点击任何按钮都会返回错误 200340。卡片看起来是正常的——只有当用户与之交互时，错误才会显现。
:::
...
对于基于文本的小型文档（.txt, .md），文件内容会自动注入到消息文本中，以便 Agent 无需借助工具即可直接读取。
...
如果飞书 API 拒绝了 post 负载（例如，由于不支持的 Markdown 结构），适配器会自动回退为发送剥离了 Markdown 的纯文本。这种两阶段回退机制确保了消息总能送达。
...
用户对机器人消息的表情回应也会被跟踪。如果用户在机器人发送的消息上添加或移除了表情回应，该事件会被路由为一个合成的文本事件（`reaction:added:EMOJI_TYPE` 或 `reaction:removed:EMOJI_TYPE`），以便 Agent 能够响应用户的反馈。

## 突发保护与批处理

适配器包含针对快速消息突发的防抖处理，以避免压垮 Agent：
...
适配器会跟踪每个 IP 地址的连续错误响应。在 6 小时窗口内，来自同一 IP 的连续错误达到 25 次后，会记录一条警告日志。这有助于检测配置错误的客户端或探测尝试。
...
入站消息使用消息 ID 进行去重，并设置 24 小时 TTL。去重状态在重启后持久化到 `~/.hermes/feishu_seen_message_ids.json`。
...
| `HERMES_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | 最大跟踪的去重消息 ID 数 |
...
| 发送的消息显示为纯文本 | 飞书 API 拒绝了消息负载；这是正常的回退行为。请查看日志了解详情。 |
...
| 点击审批按钮时出现 Error 200340 | 在飞书开发者控制台启用 **交互卡片** 能力并配置 **卡片请求地址**。参见上文的 [必需的飞书应用配置](#required-feishu-app-configuration)。 |
