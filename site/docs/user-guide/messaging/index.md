...
通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、钉钉、飞书/Lark、企业微信、微信、BlueBubbles (iMessage) 或你的浏览器与 Hermes 聊天。网关是一个单一的后台进程，连接到所有你配置的平台，处理会话，运行定时任务，并传递语音消息。

要使用完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅[语音模式](/user-guide/features/voice-mode)和[在 Hermes 中使用语音模式](/guides/use-voice-mode-with-hermes)。
...
**语音** = TTS 音频回复和/或语音消息转录。**图片** = 发送/接收图片。**文件** = 发送/接收文件附件。**线程** = 线程化对话。**反应** = 消息上的表情符号反应。**输入中** = 处理时显示输入指示器。**流式传输** = 通过编辑实现渐进式消息更新。
...
这将引导你通过方向键选择来配置每个平台，显示哪些平台已配置，并在完成后提供启动/重启网关的选项。
...
**默认情况下，网关拒绝所有不在允许列表中或未通过私信配对的用户。** 这是对于具有终端访问权限的机器人的安全默认设置。
...
display:
  ...
  tool_progress_command: false  # 设为 true 以在消息传递中启用 /verbose
```
...
在单独的后台会话中运行提示，这样 Agent 可以独立处理任务，而你的主聊天会话保持响应：
...
每个 `/background` 提示都会生成一个**独立的 Agent 实例**，该实例异步运行：
...
- **结果交付** — 任务完成后，结果会发送回你发出命令的**同一聊天或频道**，并带有“✅ 后台任务完成”前缀。如果失败，你会看到“❌ 后台任务失败”以及错误信息。
...
:::tip
消息平台上的后台任务是“发射后不管”的 — 你无需等待或检查它们。任务完成后，结果会自动到达同一聊天。
:::
...
- **PATH** — 安装时你的完整 shell PATH，并预置了 venv 的 `bin/` 和 `node_modules/.bin`。这确保用户安装的工具（Node.js、ffmpeg 等）对网关子进程（如 WhatsApp 桥接）可用。
