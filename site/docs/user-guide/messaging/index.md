...
你可以通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、飞书/Lark、企业微信、微信、BlueBubbles (iMessage) 或浏览器与 Hermes 聊天。网关是一个单一的后台进程，用于连接你配置的所有平台、处理会话、运行定时任务并发送语音消息。

如需了解完整的语音功能集（包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话），请参阅 [语音模式](/user-guide/features/voice-mode) 和 [在 Hermes 中使用语音模式](/guides/use-voice-mode-with-hermes)。
...
**语音** = TTS 音频回复和/或语音消息转录。**图片** = 发送/接收图片。**文件** = 发送/接收文件附件。**线程** = 线程化对话。**反应** = 消息上的表情符号反应。**输入状态** = 处理时的输入指示器。**流式传输** = 通过编辑消息实现的渐进式更新。
...
每个平台适配器接收消息，通过按聊天会话存储进行路由，并将其分发给 AIAgent 进行处理。网关还运行定时任务调度器，每 60 秒触发一次以执行到期的任务。
...
该向导会引导你通过方向键选择来配置每个平台，显示已配置的平台，并在完成后提供启动/重启网关的选项。
...
| `/approve` | 批准待处理的危险命令 |
| `/deny` | 拒绝待处理的危险命令 |
| `/sethome` | 将当前聊天设置为家庭频道 |
| `/compress` | 手动压缩对话上下文 |
| `/title [name]` | 设置或显示会话标题 |
| `/resume [name]` | 恢复之前命名的会话 |
| `/usage` | 显示当前会话的 Token 使用量 |
...
会话在消息之间保持持久，直到重置。Agent 会记住你的对话上下文。
...
**默认情况下，网关会拒绝所有不在允许列表或未通过私信（DM）配对的用户。** 对于具有终端访问权限的机器人来说，这是安全的默认设置。
...
### 私信（DM）配对（允许列表的替代方案） {#dm-pairing-alternative-to-allowlists}

未知用户在私信机器人时，会收到一个一次性配对码，而不是手动配置用户 ID：
...
在独立的后台会话中运行提示词，这样 Agent 就可以独立处理任务，而不会影响主聊天窗口的响应能力：
...
- **隔离会话** — 后台 Agent 拥有自己的会话和对话历史。它无法感知你当前的聊天上下文，仅接收你提供的提示词。
- **相同配置** — 继承当前网关设置中的模型、提供商、工具集、推理设置和提供商路由。
- **非阻塞** — 你的主聊天窗口保持完全可交互。在后台任务运行的同时，你可以发送消息、运行其他命令或启动更多后台任务。
- **结果交付** — 任务完成后，结果会发送回你发出命令的**同一个聊天或频道**，并带有“✅ Background task complete”前缀。如果任务失败，你会看到带有错误信息的“❌ Background task failed”。
...
- **长时间构建** — “/background Build and deploy the staging environment”，同时你可以继续聊天
...
:::tip
消息平台上的后台任务属于“即发即忘”模式——你无需等待或手动检查。任务完成后，结果会自动出现在同一个聊天窗口中。
:::
...
在笔记本电脑和开发机上使用用户服务。在 VPS 或无头主机上使用系统服务，这样无需依赖 systemd linger 即可在开机时自动恢复运行。

除非有特殊需求，否则请避免同时安装用户级和系统级网关单元。如果 Hermes 检测到两者同时存在，会发出警告，因为 start/stop/status 的行为会变得不明确。

:::info 多重安装
如果你在同一台机器上运行多个 Hermes 安装（使用不同的 `HERMES_HOME` 目录），每个安装都会拥有自己的 systemd 服务名称。默认的 `~/.hermes` 使用 `hermes-gateway`；其他安装则使用 `hermes-gateway-<hash>`。`hermes gateway` 命令会自动定位到当前 `HERMES_HOME` 对应的正确服务。
:::
...
:::info 多重安装
与 Linux 的 systemd 服务一样，每个 `HERMES_HOME` 目录都会获得自己的 launchd 标签。默认的 `~/.hermes` 使用 `ai.hermes.gateway`；其他安装则使用 `ai.hermes.gateway-<suffix>`。
:::
...
- [微信 (Weixin) 设置](weixin.md)
- [BlueBubbles (iMessage) 设置](bluebubbles.md)
