...
Hermes 可以使用标准的 IMAP 和 SMTP 协议接收和回复电子邮件。向代理的邮箱地址发送邮件，它会在同一邮件线程中回复——无需特殊的客户端或机器人 API。适用于 Gmail、Outlook、Yahoo、Fastmail 或任何支持 IMAP/SMTP 的提供商。

:::info 无外部依赖
电子邮件适配器使用 Python 内置的 `imaplib`、`smtplib` 和 `email` 模块。无需额外的软件包或外部服务。
:::
...
适配器以可配置的间隔（默认：15 秒）轮询 IMAP 收件箱中的 UNSEEN（未读）邮件。对于每封新邮件：

- **主题行** 会作为上下文包含（例如 `[Subject: Deploy to production]`）
- **回复邮件**（主题以 `Re:` 开头）会跳过主题前缀——因为邮件线程上下文已建立
- **附件** 会缓存在本地：
  - 图片（JPEG、PNG、GIF、WebP）→ 可供视觉工具使用
  - 文档（PDF、ZIP 等）→ 可供文件访问工具使用
- **纯 HTML 邮件** 会剥离标签以提取纯文本
- **自己发送给自己的邮件** 会被过滤掉，以防止回复循环
- **自动/无回复发件人** 会被静默忽略——包括 `noreply@`、`mailer-daemon@`、`bounce@`、`no-reply@` 等地址，以及带有 `Auto-Submitted`、`Precedence: bulk` 或 `List-Unsubscribe` 邮件头的邮件
...
代理可以在回复中发送文件附件。在响应中包含 `MEDIA:/path/to/file`，该文件就会附加到外发邮件中。
...
:::warning
**务必配置 `EMAIL_ALLOWED_USERS`。** 如果不配置，任何知道代理邮箱地址的人都可以发送命令。默认情况下，代理拥有终端访问权限。
:::
...
| 启动时出现 **"SMTP connection failed"** | 验证 `EMAIL_SMTP_HOST` 和 `EMAIL_SMTP_PORT`。检查密码是否正确（对于 Gmail 请使用应用专用密码）。 |
| **未收到消息** | 检查 `EMAIL_ALLOWED_USERS` 是否包含发件人的邮箱地址。检查垃圾邮件文件夹——有些提供商会标记自动回复。 |
| **"Authentication failed"** | 对于 Gmail，必须使用应用专用密码，而不是常规密码。确保已先启用双重认证。 |
| **重复回复** | 确保只有一个网关实例在运行。检查 `hermes gateway status`。 |
| **响应缓慢** | 默认轮询间隔为 15 秒。可以通过设置 `EMAIL_POLL_INTERVAL=5` 来减少间隔以获得更快的响应（但会增加 IMAP 连接数）。 |
...
:::warning
**使用专用的电子邮件账户。** 不要使用你的个人邮箱——代理会将密码存储在 `.env` 文件中，并通过 IMAP 拥有完整的收件箱访问权限。
:::

- 使用**应用专用密码**代替你的主密码（对于启用了双重认证的 Gmail 是必需的）
- 设置 `EMAIL_ALLOWED_USERS` 以限制可以与代理交互的人员
