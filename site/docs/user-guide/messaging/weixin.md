...
将 Hermes 连接到腾讯的个人即时通讯平台 [微信](https://weixin.qq.com/)。此适配器使用腾讯的 **iLink Bot API** 来连接个人微信账号——这与企业微信（WeCom）不同。消息通过长轮询方式传递，因此不需要公共端点或 Webhook。

:::info
此适配器适用于**个人微信账号**。如果你需要连接企业微信，请改用 [WeCom 适配器](./wecom.md)。
:::
...
- **上下文令牌持久化** — 跨重启的磁盘备份回复连续性
- **Markdown 格式化** — 标题、表格和代码块会重新格式化以提高在微信中的可读性
- **智能消息分块** — 消息在限制内保持为单个气泡；仅超长负载会在逻辑边界处拆分
- **输入状态指示器** — 在 Agent 处理时，微信客户端显示“正在输入…”状态
...
| `token` | — | iLink Bot 令牌（必需，从二维码登录自动保存） |
...
| `split_multiline_messages` | `false` | 为 `true` 时，将多行回复拆分为多个聊天消息（旧版行为）。为 `false` 时，将多行回复保持为一条消息，除非超过长度限制。 |
...
:::note
微信的默认群聊策略是 `disabled`（不同于企业微信默认的 `open`）。这是有意为之，因为个人微信账号可能加入许多群组。
:::
...
- **出站：** 文件在本地使用随机的 AES-128-ECB 密钥加密，上传到 CDN，并将加密引用包含在出站消息中。
...
iLink Bot API 要求为给定对等方的每条出站消息回显一个 `context_token`。适配器维护一个磁盘备份的上下文令牌存储：
...
只要消息长度在平台限制内，就会作为单条聊天消息发送。只有超大的消息负载才会被拆分发送：

- 最大消息长度：**4000 个字符**
- 即使包含多个段落或换行，未超限的消息也会保持完整
- 超长消息会在逻辑边界（段落、空行、代码块）处拆分
- 代码块会尽可能保持完整（除非代码块本身超过限制，否则不会在块内拆分）
- 超大的单个代码块会回退到基础适配器的截断逻辑
- 发送多个分块时，会设置 0.3 秒的分块间隔，以防止触发微信的速率限制
...
入站消息会使用消息 ID 进行去重，时间窗口为 5 分钟。这可以防止在网络波动或轮询响应重叠时重复处理消息。

### Token 锁

同一时间只能有一个微信网关实例使用给定的 token。适配器在启动时会获取一个作用域锁，并在关闭时释放。如果另一个网关已经在使用相同的 token，启动将失败并显示提示性错误信息。
...
| `WEIXIN_ALLOWED_USERS` | — | _(空)_ | 私聊白名单的逗号分隔用户 ID |
| `WEIXIN_GROUP_ALLOWED_USERS` | — | _(空)_ | 群组白名单的逗号分隔群组 ID |
| `WEIXIN_HOME_CHANNEL` | — | — | 定时任务/通知输出的聊天 ID |
| `WEIXIN_HOME_CHANNEL_NAME` | — | `Home` | 主频道的显示名称 |
| `WEIXIN_ALLOW_ALL_USERS` | — | — | 允许所有用户的网关级标志（由设置向导使用） |
...
| 设置过程中二维码过期 | 二维码最多会自动刷新 3 次。如果持续过期，请检查网络连接 |
...
| 媒体下载/上传失败 | 确保已安装 `cryptography`。检查网络是否能访问 `novac2c.cdn.weixin.qq.com` |
...
| `iLink POST ... HTTP 4xx/5xx` | iLink 服务的 API 错误。检查你的 token 有效性和网络连接 |
| 终端二维码不显示 | 安装 `qrcode`：`pip install qrcode`。或者，打开二维码上方打印的 URL |
