...
将 Hermes 连接到腾讯的个人即时通讯平台 [微信](https://weixin.qq.com/)。该适配器使用腾讯的 **iLink Bot API** 来连接个人微信账号——这与企业微信不同。消息通过长轮询方式传递，因此不需要公共端点或 Webhook。

:::info
此适配器适用于**个人微信账号**。如果你需要企业微信，请改用 [WeCom 适配器](./wecom.md)。
:::
...
- 当 Hermes 安装了 `messaging` 额外组件时，终端二维码渲染功能已包含在内
...
- **智能消息分块** — 消息在限制内保持为单个气泡；只有超大的负载才会在逻辑边界处拆分
...
:::note
对于微信，默认的群聊策略是 `disabled`（与企业微信默认 `open` 不同）。这是有意为之，因为个人微信账号可能加入了许多群组。
:::

## 媒体支持 {#media-support}

### 接收 {#inbound-receiving}
...
- **接收：** 使用 `encrypted_query_param` URL 从 CDN 下载加密媒体，然后使用消息负载中提供的每个文件的密钥通过 AES-128-ECB 解密。
- **发送：** 文件在本地使用随机 AES-128-ECB 密钥加密，上传到 CDN，加密后的引用包含在发送的消息中。
...
所有发送的媒体都经过加密 CDN 上传流程：

1. 生成随机 AES-128 密钥
2. 使用 AES-128-ECB + PKCS#7 填充加密文件
3. 向 iLink API (`getuploadurl`) 请求上传 URL
4. 将密文上传到 CDN
5. 发送包含加密媒体引用的消息

## 上下文令牌持久化 {#context-token-persistence}

iLink Bot API 要求每个出站消息都回显对应特定对等方的 `context_token`。适配器维护一个基于磁盘的上下文令牌存储：
...
- 最大消息长度：**4000 字符**
- 未超限的消息即使包含多个段落或换行也会保持完整
- 超长消息会在逻辑边界处（段落、空行、代码块）进行分割
- 代码块会尽可能保持完整（除非代码块本身超过限制，否则不会在块内分割）
- 超长的单个块会回退到基础适配器的截断逻辑
- 发送多个分块时，会添加 0.3 秒的分块间隔，以防止触发微信的速率限制
...
同一时间只能有一个 Weixin 网关实例使用给定的 token。适配器在启动时会获取一个作用域锁，并在关闭时释放。如果另一个网关已经在使用相同的 token，启动会失败并显示提示性错误信息。
...
| 设置过程中二维码过期 | 二维码会自动刷新最多 3 次。如果持续过期，请检查你的网络连接 |
...
| 消息出现重复 | 适配器通过消息 ID 去重。如果看到重复，请检查是否有多个网关实例在运行 |
| `iLink POST ... HTTP 4xx/5xx` | iLink 服务的 API 错误。检查你的 token 有效性和网络连接 |
| 终端二维码无法渲染 | 重新安装并包含 messaging 额外依赖：`pip install hermes-agent[messaging]`。或者，打开二维码上方打印的 URL |
