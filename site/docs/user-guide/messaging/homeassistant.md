...
:::info
当设置了 `HASS_TOKEN` 时，`homeassistant` 工具集会自动启用。网关平台和设备控制工具都会通过这个单一令牌激活。
:::
...
Home Assistant 网关适配器通过 WebSocket 连接并订阅 `state_changed` 事件。当设备状态发生变化且符合您的筛选条件时，它会作为消息转发给代理。
...
在 `~/.hermes/gateway.json` 中 Home Assistant 平台的 `extra` 部分下配置代理能看到哪些事件：
...
来自代理的出站消息将作为 **Home Assistant 持久通知**（通过 `persistent_notification.create`）发送。这些通知会出现在 HA 通知面板中，标题为 "Hermes Agent"。
...
-   **授权** — HA 事件始终经过授权（不需要用户白名单，因为 `HASS_TOKEN` 会对连接进行身份验证）
...
实体 ID 会针对模式 `^[a-z_][a-z0-9_]*\.[a-z0-9_]+$` 进行验证，以防止注入攻击。
