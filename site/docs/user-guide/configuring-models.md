...
本页介绍如何从控制面板配置这两种模型。如果你更喜欢配置文件或 CLI，请跳到底部的[其他方法](#alternative-methods)。

## 模型页面 {#the-models-page}

打开控制面板，点击侧边栏中的 **Models**。你会看到两个区域：

1. **模型设置（Model Settings）**——顶部面板，用于为插槽分配模型。
2. **用量分析（Usage analytics）**——排名卡片，显示所选时间段内运行过的每个模型，包含 token 计数、成本和能力徽章。

![模型页面概览](/img/docs/dashboard-models/overview.png)

最上面的卡片是 **模型设置** 面板。主行始终显示 Agent 为新会话启动时所用的模型。点击 **Change** 打开选择器。

## 设置主模型 {#setting-the-main-model}
...
- **右侧**——所选提供商的精选模型列表。这些是 Hermes 为该提供商推荐的 Agent 模型，而不是原始的 `/models` 列表（在 OpenRouter 上，原始列表包括 400 多个模型，含 TTS、图像生成器和重排序器）。
...
## 设置辅助模型 {#setting-auxiliary-models}
...
### 常见覆盖模式 {#common-override-patterns}
...
## "Use as" 快捷方式 {#the-use-as-shortcut}
...
## 写入 `config.yaml` 的内容 {#what-gets-written-to-config-yaml}
...
## 何时生效？ {#when-does-it-take-effect}

- **CLI**（`hermes chat`）：下次调用 `hermes chat` 时生效。
- **Gateway**（Telegram、Discord、Slack 等）：下一个*新*会话生效。已有会话保持原有模型。如果你想强制所有会话都应用更改，请重启 gateway（`hermes gateway restart`）。
...
### 选择器中显示"No authenticated providers" {#no-authenticated-providers-in-the-picker}
...
### 正在运行的聊天中主模型没有变化 {#main-model-didn-t-change-in-my-running-chat}

这是预期的行为。仪表盘写入 `config.yaml`，新会话会读取它。当前打开的聊天是一个正在运行的 Agent 进程——它会保留启动时使用的模型。在聊天中使用 `/model &lt;name&gt;` 可以热切换该特定会话的模型。

### 辅助覆盖“没有生效” {#auxiliary-override-didn-t-take-effect}

请检查以下三点：

1. **你是否启动了新会话？** 已有的聊天不会重新读取配置。
2. **`provider` 是否设置为 `auto` 以外的值？** 如果该字段显示为 `auto`，则任务仍在使用你的主模型。点击 **Change** 并选择一个真实的提供商。
3. **提供商是否已认证？** 如果你将 `minimax` 分配给某个任务，但没有 MiniMax API 密钥，该任务会回退到 openrouter 默认值，并在 `agent.log` 中记录一条警告。

### 我选了一个模型，但 Hermes 给我切换了提供商 {#i-picked-a-model-but-hermes-switched-providers-on-me}
...
### `hermes model` 子命令 {#hermes-model-subcommand}
...
### 直接编辑配置 {#direct-config-edit}

编辑 `~/.hermes/config.yaml` 并重启任何读取它的进程。完整模式请参见[配置参考](./configuration.md)。
