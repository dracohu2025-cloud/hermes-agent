...
Hermes Agent 的 CLI 是一个完整的终端用户界面（TUI）—— 不是 Web UI。它具备多行编辑、斜杠命令自动补全、对话历史、中断与重定向以及流式工具输出等功能。专为生活在终端里的人打造。
...
Hermes 会在第一轮对话之前将每个命名的技能加载到会话提示中。相同的标志在交互模式和单次查询模式下都有效。

## 技能斜杠命令

`~/.hermes/skills/` 中的每个已安装技能都会自动注册为斜杠命令。技能名称即成为命令：
...
-   当 Agent 正在工作时**输入新消息 + Enter** —— 它会中断并处理您的新指令
...
`display.busy_input_mode` 配置键控制当您在 Agent 工作时按下 Enter 时发生的情况：

| 模式 | 行为 |
|------|----------|
| `"interrupt"` (默认) | 您的消息会中断当前操作并立即被处理 |
| `"queue"` | 您的消息会被静默排队，并在 Agent 完成后作为下一轮对话发送 |
```yaml
# ~/.hermes/config.yaml
display:
...
队列模式在你想要准备后续消息，同时避免意外取消正在执行的工作时很有用。未知的值会回退到 `"interrupt"`。

### 挂起到后台

在 Unix 系统上，按 **`Ctrl+Z`** 可以将 Hermes 挂起到后台——就像任何终端进程一样。Shell 会打印确认信息：

```
Hermes Agent 已被挂起。运行 `fg` 可将 Hermes Agent 带回前台。
```

在 shell 中输入 `fg` 即可恢复到你离开时的会话状态。Windows 不支持此功能。
...
使用 `/verbose` 命令循环切换显示模式：`off → new → all → verbose`。此命令也可在消息平台上启用——参见[配置](/user-guide/configuration#display-settings)。

### 工具预览长度

配置项 `display.tool_preview_length` 控制工具调用预览行（例如文件路径、终端命令）中显示的最大字符数。默认值为 `0`，表示无限制——会显示完整路径和命令。
...
在聊天中使用 `/title My Session Name` 来命名当前会话，或在命令行使用 `hermes sessions rename &lt;id&gt; &lt;title&gt;`。使用 `hermes sessions list` 来浏览过去的会话。
...
│ 3. 在 14:30 时来自 192.168.1.50 的 SSH 登录尝试失败      │
╰──────────────────────────────────────────────────────────────╯
```

如果任务失败，你会看到一个错误通知。如果你的配置中启用了 `display.bell_on_complete`，任务完成时终端会响铃。
