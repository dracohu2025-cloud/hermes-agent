...
- 在代理工作时**输入新消息 + Enter** —— 它会中断并处理您的新指令
...
恢复操作会从 SQLite 中还原完整的对话历史。代理会看到所有先前的消息、工具调用和响应 —— 就像您从未离开过一样。
...
- **隔离的对话** —— 背景代理不知道您当前会话的历史。它只接收您提供的提示。
- **相同的配置** —— 背景代理继承您当前会话的模型、提供商、工具集、推理设置和回退模型。
