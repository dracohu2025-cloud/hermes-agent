...
Hermes Agent 会自动将每次对话保存为会话。会话功能支持恢复对话、跨会话搜索以及完整的对话历史管理。

## 会话的工作原理

无论是来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix 还是其他任何消息平台，每一次对话都会作为包含完整消息历史的会话进行存储。会话通过以下两个互补的系统进行追踪：
...
此操作会从 SQLite 数据库中查找最近的 `cli` 会话并加载其完整的对话历史。

### 按名称恢复

如果你为会话设置了标题（请参阅下方的 [会话命名](#session-naming)），则可以通过名称恢复它：

```bash
# 恢复已命名的会话
hermes -c "my project"

# 如果存在多个变体（my project, my project #2, my project #3），
# 此操作会自动恢复最近的一个
...
会话 ID 会在退出 CLI 会话时显示，也可以通过 `hermes sessions list` 命令查找。
...
<p className="docs-figure-caption">恢复模式会在返回实时提示符之前，显示一个包含近期用户和 Agent 交互的简要回顾面板。</p>

回顾内容说明：
- 显示**用户消息**（金色 `●`）和 **Agent 回复**（绿色 `◆`）
- **截断**过长的消息（用户消息 300 字符，Agent 消息 200 字符/3 行）
- **折叠**工具调用，仅显示工具名称计数（例如 `[3 tool calls: terminal, web_search]`）
- **隐藏**系统消息、工具结果和内部推理过程
- **限制**在最近的 10 次交互，并带有“... N 条更早的消息 ...”提示
- 使用**暗色样式**以区别于当前正在进行的对话
若要禁用摘要并保持简洁的单行显示行为，请在 `~/.hermes/config.yaml` 中进行如下设置：
...
:::tip
Session ID 的格式为 `YYYYMMDD_HHMMSS_<8位十六进制字符>`，例如 `20250305_091523_a1b2c3d4`。你可以通过 ID 或标题来恢复会话——两者均适用于 `-c` 和 `-r` 参数。
:::

## 会话命名 {#session-naming}

为会话设置易于理解的标题，以便你能轻松查找和恢复它们。

### 自动生成标题

在首次交互后，Hermes 会自动为每个会话生成一个简短的描述性标题（3–7 个单词）。此过程在后台线程中使用快速辅助模型运行，因此不会增加任何延迟。当你使用 `hermes sessions list` 或 `hermes sessions browse` 浏览会话时，就能看到自动生成的标题。
...
标题会立即生效。如果会话尚未在数据库中创建（例如，你在发送第一条消息之前运行了 `/title`），它会被加入队列，并在会话开始后应用。
...
- **唯一性** — 不允许两个会话使用相同的标题
- **最大 100 个字符** — 保持列表输出整洁
- **已清理** — 控制字符、零宽字符和 RTL 覆盖字符会被自动剔除
- **支持常规 Unicode** — 支持 Emoji、CJK（中日韩统一表意文字）、带重音的字符等
...
当你按名称恢复会话（`hermes -c "my project"`）时，它会自动选择该谱系中最近的一个会话。
...
如果该标题已被其他会话使用，系统会显示错误。

### 清理旧会话

```bash
# 删除 90 天（默认值）之前已结束的会话
...
如需更深入的分析（如 Token 使用量、成本估算、工具调用细分和活跃度模式），请使用 [`hermes insights`](/reference/cli-commands#hermes-insights)。

## 会话搜索工具

Agent 内置了一个 `session_search` 工具，它使用 SQLite 的 FTS5 引擎对所有过往对话进行全文搜索。

### 工作原理

1. FTS5 根据相关性对匹配的消息进行排序搜索
2. 按会话对结果进行分组，取前 N 个唯一会话（默认为 3 个）
3. 加载每个会话的对话，截取匹配内容周围约 100K 字符的上下文
4. 发送给快速摘要模型进行重点总结
5. 返回包含元数据和周边上下文的各会话摘要
...
> *"当用户引用过往对话中的内容，或者你怀疑存在相关的先前上下文时，请在要求用户重复之前使用 session_search 进行回顾。"*
...
当 Hermes 无法获取共享聊天中的参与者标识符时，它会回退到为该房间使用一个共享会话。
...
- 一个用户繁重的工具任务不会污染另一个用户的上下文窗口
- 中断处理也保持在用户级别，因为运行中的 Agent 键与独立的会话键相匹配

如果你希望改为一个共享的“房间大脑”，请设置：

```yaml
group_sessions_per_user: false
```

这将使群组/频道恢复为每个房间一个共享会话，这样可以保留共享的对话上下文，但也会共享 Token 成本、中断状态和上下文增长。
### 会话重置策略
...
- **idle** — 在闲置 N 分钟后重置
- **daily** — 每天在特定小时重置
- **both** — 满足任一条件（闲置或每日）即重置
- **none** — 从不自动重置

在会话自动重置之前，Agent 会获得一个回合的机会，用于保存对话中重要的记忆或技能。

无论策略如何，拥有**活跃后台进程**的会话永远不会被自动重置。
...
| Gateway 转录记录 | `~/.hermes/sessions/` | 每个会话的 JSONL 转录记录 + sessions.json 索引 |
| Gateway 索引 | `~/.hermes/sessions/sessions.json` | 将会话键映射到活跃会话 ID |

SQLite 数据库使用 WAL 模式以支持并发读取和单写入，这非常适合 Gateway 的多平台架构。
...
:::tip
数据库增长缓慢（典型情况：数百个会话仅占用 10-15 MB）。清理主要用于移除那些你不再需要用于搜索回溯的旧对话。
:::
