...
sidebar_position: 7
title: "会话"
description: "会话持久化、恢复、搜索、管理，以及各平台的会话追踪"
---
...
- **折叠**工具调用，仅显示数量和工具名（例如 `[3 tool calls: terminal, web_search]`）
- **隐藏**系统消息、工具结果和内部推理
...
如需更深入的分析——如令牌使用量、成本估算、工具使用情况和活动模式——请使用 [`hermes insights`](/reference/cli-commands#hermes-insights)。
...
3.  加载每个会话的对话内容，截取匹配项前后约 10 万个字符
4.  发送给一个快速的摘要模型以生成聚焦的摘要
5.  返回每个会话的摘要，包含元数据和相关上下文
...
> *“当用户提及过去的对话内容，或者你怀疑存在相关的先前上下文时，请使用 session_search 来回忆，而不是要求用户重复。”*
...
| WhatsApp 私聊 | `agent:main:whatsapp:dm:&lt;chat_id&gt;` | 每个私聊对应一个会话 |
| 群组聊天 | `agent:main:&lt;platform&gt;:group:&lt;chat_id&gt;:&lt;user_id&gt;` | 当平台暴露用户 ID 时，群组内每个用户一个会话 |
| 群组线程/话题 | `agent:main:&lt;platform&gt;:group:&lt;chat_id&gt;:&lt;thread_id&gt;` | 线程内所有参与者共享一个会话（默认）。如果设置 `thread_sessions_per_user: true`，则为每个用户一个会话。 |
...
SQLite 数据库使用 WAL 模式，支持并发读取和单一写入，这非常适合网关的多平台架构。

### 数据库模式

`state.db` 中的关键表：

-   **sessions** — 会话元数据（id、来源、user_id、模型、标题、时间戳、令牌计数）。标题有唯一索引（允许 NULL 标题，仅非 NULL 标题必须唯一）。
...
:::tip
数据库增长缓慢（典型情况：数百个会话占用 10-15 MB）。清理主要用于删除那些你不再需要用于搜索回忆的旧对话。
:::
