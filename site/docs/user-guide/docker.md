...
`/opt/data` 卷是 Hermes 所有状态的单一事实来源。它映射到宿主机上的 `~/.hermes/` 目录，包含：
...
:::warning
切勿同时运行两个 Hermes 容器指向同一个数据目录 — 会话文件和记忆存储并非为并发访问设计。
:::
...
当使用 Docker 作为执行环境时（不是上述方法，而是指 Agent 在 Docker 沙箱内运行命令），Hermes 会自动将技能目录 (`~/.hermes/skills/`) 和技能声明的任何凭证文件作为只读卷绑定挂载到容器中。这意味着技能脚本、模板和引用在沙箱内可用，无需手动配置。
