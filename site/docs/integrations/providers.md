...
:::info Codex 说明
OpenAI Codex 提供商通过设备码进行认证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的认证存储中，位于 `~/.hermes/auth.json`，并且当存在时可以从 `~/.codex/auth.json` 导入现有的 Codex CLI 凭据。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用一个独立的“辅助”模型——默认是通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 会自动启用这些工具。你也可以配置这些工具使用哪个模型和提供商——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::
...
# Hermes 在可用时会直接使用 Claude Code 的凭据存储
hermes model
...
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 会优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env`。这可以保持可刷新的 Claude 凭据可刷新。
...
这些提供商具有内置支持，并有专用的提供商 ID。设置 API 密钥并使用 `--provider` 进行选择：
...
```yaml
model:
  provider: "zai"       # 或：kimi-coding, kimi-coding-cn, minimax, minimax-cn, alibaba, xiaomi
  default: "glm-5"
```

基础 URL 可以通过 `GLM_BASE_URL`、`KIMI_BASE_URL`、`MINIMAX_BASE_URL`、`MINIMAX_CN_BASE_URL`、`DASHSCOPE_BASE_URL` 或 `XIAOMI_BASE_URL` 环境变量进行覆盖。
:::note Z.AI 端点自动探测
使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、代码专用版）以找到接受你 API 密钥的那个。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动探测并缓存。
:::
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取你的令牌 —— 确保启用 "Make calls to Inference Providers" 权限。包含免费额度（每月 $0.10 信用额度，提供商费率无加价）。
...
:::tip
`/model custom`（裸命令，不带模型名称）会查询你端点的 `/models` API，如果恰好加载了一个模型，则自动选择它。对于运行单个模型的本地服务器很有用。
:::
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认 0.9）以将更多上下文挤入显存。
...
:::tip
vLLM 支持人类可读的大小格式：`--max-model-len 64k`（小写 k 表示 1000，大写 K 表示 1024）。
:::

---

### SGLang — 使用 RadixAttention 实现快速服务

[SGLang](https://github.com/sgl-project/sglang) 是 vLLM 的替代方案，它利用 RadixAttention 实现 KV 缓存复用。最适合：多轮对话（前缀缓存）、受约束解码、结构化输出。
...
**工具调用：** 使用 `--tool-call-parser` 并为你模型系列选择合适的解析器：`qwen` (Qwen 2.5)、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。如果没有这个标志，工具调用将以纯文本形式返回。

:::caution SGLang 默认最大输出令牌数为 128
如果响应看起来被截断了，请在请求中添加 `max_tokens` 参数，或在服务器上设置 `--default-max-tokens`。如果请求中没有指定，SGLang 的默认值仅为每个响应 128 个令牌。
:::
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文长度。对于训练上下文为 128k+ 的模型，尝试分配完整的 KV 缓存可能会导致内存不足 (OOM)。请根据你的需要明确设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位 (`-np`)，总上下文长度会在槽位间分配 —— 使用 `-c 32768 -np 4` 时，每个槽位只有 8k。
...
LM Studio 从模型的元数据中读取上下文长度，但许多 GGUF 模型报告较低的默认值（2048 或 4096）。**务必在 LM Studio 模型设置中明确设置上下文长度**：

1. 点击模型选择器旁边的齿轮图标
2. 将“上下文长度”设置为至少 16384（最好是 32768）
3. 重新加载模型以使更改生效
...
:::tip 两者都在 WSL2 内？没问题。
如果你的模型服务器也运行在 WSL2 内（vLLM、SGLang 和 llama-server 常见），`localhost` 可以正常工作 —— 它们共享相同的网络命名空间。跳过本节。
:::
...
**Windows 上的 Ollama（详细说明）：** Ollama 作为 Windows 服务运行。设置 `OLLAMA_HOST` 的方法：
1. 打开 **系统属性** → **环境变量**
...
Windows 防火墙将 WSL2 视为一个独立的网络（无论是 NAT 还是镜像模式）。如果按照上述步骤操作后连接仍然失败，请为你的模型服务器端口添加防火墙规则：
...
如果能收到列出模型的 JSON 响应，说明没问题。在 Hermes 配置中使用相同的 URL 作为 `base_url`。

---

### 本地模型故障排除

这些问题在将 Hermes 与 **所有** 本地推理服务器一起使用时都可能遇到。

#### 从 WSL2 到 Windows 主机上的模型服务器出现“Connection refused”

如果你在 WSL2 内运行 Hermes，而模型服务器在 Windows 主机上运行，在 WSL2 默认的 NAT 网络模式下 `http://localhost:&lt;port&gt;` 将无法工作。请参阅上文的 [WSL2 网络](#wsl2-networking-windows-users) 部分进行修复。

#### 工具调用以文本形式出现而不是执行

模型输出类似 `{"name": "web_search", "arguments": {...}}` 的内容作为消息，而不是实际调用工具。

**原因：** 你的服务器没有启用工具调用功能，或者模型不支持通过该服务器的工具调用实现。
...
| **Ollama** | 默认启用工具调用 — 确保你的模型支持它（用 `ollama show model-name` 检查） |
| **LM Studio** | 更新到 0.3.6+ 版本并使用具有原生工具支持的模型 |

#### 模型似乎忘记上下文或给出不连贯的回复

**原因：** 上下文窗口太小。当对话超过上下文限制时，大多数服务器会静默丢弃较早的消息。Hermes 的系统提示词加上工具 schema 本身就可能占用 4k–8k tokens。
...
**修复：** 为 Agent 使用，至少设置 **32,768 tokens** 的上下文。查看上面每个服务器的具体章节了解对应的参数设置。

#### 启动时显示“Context limit: 2048 tokens”

Hermes 会自动从你的服务器的 `/v1/models` 端点检测上下文长度。如果服务器报告的值很低（或根本不报告），Hermes 会使用模型声明的限制，这可能是不正确的。
...
**可能的原因：**
1. **服务器上的输出上限 (`max_tokens`) 较低** — SGLang 默认每个回复 128 tokens。在服务器上设置 `--default-max-tokens`，或在 config.yaml 中配置 Hermes 的 `model.max_tokens`。注意：`max_tokens` 只控制回复长度 — 它与你的对话历史能有多长无关（那是 `context_length`）。
...
[LiteLLM](https://docs.litellm.ai/) 是一个 OpenAI 兼容的代理，它将 100+ LLM 提供商统一到一个 API 后面。最适合：在不更改配置的情况下切换提供商、负载均衡、备用链、预算控制。
...
然后用 `hermes model` → 自定义端点 → `http://localhost:4000/v1` 来配置 Hermes。

带有备用链的示例 `litellm_config.yaml`：
```yaml
model_list:
  - model_name: "best"
    litellm_params:
...
由 BlockRunAI 开发的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，它根据查询复杂度自动选择模型。它在 14 个维度上对请求进行分类，并路由到能处理任务的最便宜模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
然后用 `hermes model` → 自定义端点 → `http://localhost:8402/v1` → 模型名称 `blockrun/auto` 来配置 Hermes。
...
:::note
ClawRouter 需要一个在 Base 或 Solana 上充有 USDC 的钱包进行支付。所有请求都会通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 来检查钱包状态。
:::
...
通过 `hermes model` → 自定义端点或在 `config.yaml` 中配置任何一个：
...
:::note 两个容易混淆的设置
**`context_length`** 是**总上下文窗口**——输入*和*输出 token 的共享预算（例如 Claude Opus 4.6 是 200,000）。Hermes 用它来决定何时压缩历史记录，并验证 API 请求。

**`model.max_tokens`** 是**输出上限**——模型在*单次响应*中最多能生成的 token 数量。它与你的对话历史能有多长无关。行业标准名称 `max_tokens` 是常见的混淆来源；Anthropic 的原生 API 后来将其重命名为 `max_output_tokens` 以更清晰。

当自动检测获取的窗口大小错误时，设置 `context_length`。
只有当你需要限制单次响应的长度时，才设置 `model.max_tokens`。
:::

Hermes 使用多源解析链来检测你的模型和提供者的正确上下文窗口：

1. **配置覆盖** — `config.yaml` 中的 `model.context_length`（最高优先级）
2. **自定义提供者按模型设置** — `custom_providers[].models.&lt;id&gt;.context_length`
...
如果你使用多个自定义端点（例如，本地开发服务器和远程 GPU 服务器），可以在 `config.yaml` 中将它们定义为命名的自定义提供者：
...
你也可以从交互式 `hermes model` 菜单中选择命名的自定义提供者。
...
| **中文 AI 模型** | z.ai (GLM)、Kimi/Moonshot (`kimi-coding` 或 `kimi-coding-cn`)、MiniMax 或小米 MiMo（一等提供者） |
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更喜欢在本地运行 Firecrawl，可以改为将 Hermes 指向一个自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**你能获得什么：** 无需 API 密钥，无速率限制，无每页成本，完全的数据主权。
...
配置一个备份提供者:模型，当你的主模型失败时（速率限制、服务器错误、身份验证失败），Hermes 会自动切换到它：
...
---
激活后，降级方案会在会话中切换模型和提供商，同时保留对话内容。每个会话最多触发 **一次**。

支持的提供商：`openrouter`、`nous`、`openai-codex`、`copilot`、`copilot-acp`、`anthropic`、`huggingface`、`zai`、`kimi-coding`、`kimi-coding-cn`、`minimax`、`minimax-cn`、`deepseek`、`ai-gateway`、`opencode-zen`、`opencode-go`、`kilocode`、`xiaomi`、`alibaba`、`custom`。

:::tip
降级功能仅通过 `config.yaml` 配置 —— 没有对应的环境变量。关于触发条件、支持的提供商以及它与辅助任务和委派的交互，完整详情请参阅 [Fallback Providers](/user-guide/features/fallback-providers)。
:::

## 智能模型路由

可选的经济型-强劲型路由功能能让 Hermes 将复杂工作保留在主模型上，同时把非常简短/简单的对话回合发送给更经济的模型。
...
smart_model_routing:
  ...
  cheap_model:
    ...
    # api_key_env: MY_CUSTOM_KEY          # 可选的环境变量名，用于该端点 API 密钥
```

工作原理：
- 如果对话回合简短、单行且看起来不含大量代码/工具/调试内容，Hermes 可能将其路由至 `cheap_model`
- 如果对话回合看起来复杂，Hermes 则保持在您的主模型/提供商上
- 如果经济型路由无法顺利完成，Hermes 会自动回退到主模型

这设计上是保守的。它适用于快速、低风险的回合，例如：
- 简短的事实性问题
- 快速改写
- 轻量级摘要
...
在您希望降低延迟或成本，又不完全改变默认模型时，可使用此功能。
