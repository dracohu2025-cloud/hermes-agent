...
本页介绍如何为 Hermes Agent 设置推理提供商——从 OpenRouter 和 Anthropic 等云 API，到 Ollama 和 vLLM 等自托管端点，再到高级路由和回退配置。你需要至少配置一个提供商才能使用 Hermes。
...
| **Anthropic** | `hermes model` (通过 Claude Code 认证使用 Claude Pro/Max，或使用 Anthropic API 密钥，或手动设置令牌) |
...
:::info Codex 说明
OpenAI Codex 提供商通过设备码进行认证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的认证存储 `~/.hermes/auth.json` 中，并且当存在时可以从 `~/.codex/auth.json` 导入现有的 Codex CLI 凭据。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用一个单独的“辅助”模型——默认是通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 可以自动启用这些工具。你也可以配置这些工具使用哪个模型和提供商——参见 [辅助模型](/user-guide/configuration#auxiliary-models)。
:::
...
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 会优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env`。这可以保持可刷新的 Claude 凭据能够刷新。
...
model:
  ...
  default: "glm-5"
```
可以通过 `GLM_BASE_URL`、`KIMI_BASE_URL`、`MINIMAX_BASE_URL`、`MINIMAX_CN_BASE_URL`、`DASHSCOPE_BASE_URL` 或 `XIAOMI_BASE_URL` 环境变量覆盖基础 URL。

:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球版、中国版、代码版变体）以找到接受你 API 密钥的那个。你无需手动设置 `GLM_BASE_URL` —— 可用的端点会被自动检测并缓存。
:::

### xAI (Grok) 提示词缓存

当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到对话会话内的同一台服务器，使 xAI 的基础设施能够复用缓存的系统提示词和对话历史。

无需配置 —— 当检测到 xAI 端点且会话 ID 可用时，缓存会自动激活。这可以降低多轮对话的延迟和成本。
...
在 [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 获取你的令牌 —— 确保启用“Make calls to Inference Providers”权限。包含免费额度（每月 0.10 美元信用，提供商费率无加价）。
...
:::warning 遗留环境变量
`.env` 中的 `OPENAI_BASE_URL` 和 `LLM_MODEL` 已被**移除**。Hermes 的任何部分都不会读取它们 —— `config.yaml` 是模型和端点配置的唯一真实来源。如果你的 `.env` 中有过时的条目，它们会在下次运行 `hermes setup` 或配置迁移时被自动清除。请使用 `hermes model` 或直接编辑 `config.yaml`。
:::
...
切换提供商时，Hermes 会将基础 URL 和提供商持久化到配置中，以便更改在重启后仍然有效。当从自定义端点切换到内置提供商时，过时的基础 URL 会被自动清除。
...
Ollama 默认**不会**使用你模型的完整上下文窗口。根据你的显存，默认值如下：
...
对于使用工具的 Agent 场景，**你至少需要 16k–32k 的上下文**。在 4k 的情况下，仅系统提示词 + 工具模式就可能填满窗口，没有空间留给对话。
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认 0.9）可以将更多上下文挤入显存。
...
支持的解析器：`hermes`（Qwen 2.5, Hermes 2/3）、`llama3_json`（Llama 3.x）、`mistral`、`deepseek_v3`、`deepseek_v31`、`xlam`、`pythonic`。不使用这些标志，工具调用将无法工作——模型会把工具调用作为文本输出。
...
[SGLang](https://github.com/sgl-project/sglang) 是一个替代 vLLM 的方案，采用了用于 KV 缓存复用的 RadixAttention。最适合：多轮对话（前缀缓存）、受限解码、结构化输出。
...
**上下文长度：** SGLang 默认从模型的配置读取。使用 `--context-length` 来覆盖。如果需要超过模型声明的最大值，设置 `SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1`。

**工具调用：** 使用 `--tool-call-parser` 并为你模型系列选择合适的解析器：`qwen` (Qwen 2.5)、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。不使用此标志，工具调用会以纯文本形式返回。

:::caution SGLang 默认输出最大 128 个 token
如果响应似乎被截断，在你的请求中添加 `max_tokens` 或在服务器上设置 `--default-max-tokens`。如果请求中没有指定，SGLang 默认每个响应只有 128 个 token。
:::
...
**上下文长度 (`-c`)：** 最近的构建默认为 `0`，这会从 GGUF 元数据读取模型的训练上下文。对于具有 128k+ 训练上下文的模型，尝试分配完整 KV 缓存可能导致 OOM（内存不足）。根据你的需要明确设置 `-c`（32k–64k 是 Agent 使用的一个好范围）。如果使用并行槽位 (`-np`)，总上下文会被分配给槽位——使用 `-c 32768 -np 4`，每个槽位只得到 8k。
...
:::caution `--jinja` 是工具调用必需的
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型将尝试通过在响应文本中书写 JSON 来调用工具，但 Hermes 不会将其识别为工具调用——你将看到原始 JSON 如 `{"name": "web_search", ...}` 被打印为消息，而不是实际搜索。

原生工具调用支持（最佳性能）：Llama 3.x、Qwen 2.5（包括 Coder）、Hermes 2/3、Mistral、DeepSeek、Functionary。所有其他模型使用一个通用的处理程序，工作但可能效率较低。查看 [llama.cpp 函数调用文档](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) 获取完整列表。
...
[LM Studio](https://lmstudio.ai/) 是一个用于通过 GUI 运行本地模型的桌面应用。最适合：偏好可视化界面的用户、快速模型测试、macOS/Windows/Linux 上的开发者。
...
:::caution 上下文长度经常默认为 2048
LM Studio 从模型的元数据读取上下文长度，但许多 GGUF 模型报告较低的默认值（2048 或 4096）。**总是在 LM Studio 模型设置中明确设置上下文长度**：

1. 点击模型选择器旁边的齿轮图标
2. 设置“Context Length”至少为 16384（最好 32768）
3. 重新加载模型以使更改生效
...
**工具调用：** LM Studio 0.3.6 起支持。具有原生工具调用训练的模型（Qwen 2.5、Llama 3.x、Mistral、Hermes）会被自动检测并显示一个工具徽章。其他模型使用通用的后备方案，可能不太可靠。

---

### WSL2 网络配置（Windows 用户）

由于 Hermes Agent 需要一个 Unix 环境，Windows 用户在 WSL2 中运行它。如果你的模型服务器（Ollama、LM Studio 等）运行在 **Windows 主机** 上，你需要桥接网络差距——WSL2 使用一个虚拟网络适配器及其自己的子网，所以 WSL2 内的 `localhost` 指的是 Linux VM，**而不是** Windows 主机。
...
在 **Windows 11 22H2+** 可用，镜像模式使 `localhost` 在 Windows 和 WSL2 之间双向工作——最简单的修复方法。
...
:::note Hyper-V 防火墙
在某些 Windows 11 构建上，Hyper-V 防火墙默认会阻止镜像连接。如果在启用镜像模式后 `localhost` 仍然不工作，在 **管理员 PowerShell** 中运行此命令：
```powershell
...
如果无法使用镜像模式，从 WSL2 内部找到 Windows 主机 IP，并用它代替 `localhost`：
...
:::tip 动态助手
主机 IP 可能在 WSL2 重启时改变。你可以在你的 shell 中动态获取它：
```bash
...
或使用你的机器的 mDNS 名称（需要在 WSL2 中安装 `libnss-mdns`）：
```bash
...
如果你使用 **选项 2**（NAT 模式与主机 IP），Windows 上的模型服务器必须接受来自 `127.0.0.1` 之外的连接。默认情况下，大多数服务器只监听 localhost——NAT 模式中的 WSL2 连接来自不同的虚拟子网并将被拒绝。在镜像模式下，`localhost` 直接映射，因此默认的 `127.0.0.1` 绑定正常工作。
| 服务器 | 默认绑定 | 如何修复 |
|--------|-------------|------------|
...
这些问题在 Hermes 使用**所有**本地推理服务器时都可能出现。

#### 从 WSL2 到 Windows 主机上运行的模型服务器出现“连接被拒绝”

如果你在 WSL2 内运行 Hermes，而模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<端口>` 将无法工作。请参考上面的 [WSL2 网络](#wsl2-networking-windows-users) 部分进行修复。

#### 工具调用显示为文本而不是执行

模型输出了类似 `{"name": "web_search", "arguments": {...}}` 的内容作为消息，而不是实际调用工具。

**原因：** 你的服务器没有启用工具调用功能，或者模型不支持该服务器实现的工具调用。
...
| **Ollama** | 工具调用默认启用 — 确保你的模型支持它（用 `ollama show 模型名` 检查） |
...
**修复：** 为 Agent 使用设置至少 **32,768 个 token** 的上下文。请参考上面每个服务器的部分查看具体的设置标志。
...
BlockRunAI 开发的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，它根据查询复杂度自动选择模型。它通过 14 个维度对请求进行分类，并路由到能够处理任务的最便宜模型。支付通过 USDC 加密货币进行（无需 API 密钥）。
...
然后在 Hermes 中配置：`hermes model` → 自定义端点 → `http://localhost:8402/v1` → 模型名 `blockrun/auto`。
...
:::note
ClawRouter 需要一个在 Base 或 Solana 上注资了 USDC 的钱包进行支付。所有请求都通过 BlockRun 的后端 API 路由。运行 `npx @blockrun/clawrouter doctor` 来检查钱包状态。
:::
...
:::note 两个容易混淆的设置
**`context_length`** 是**总上下文窗口**——输入*和*输出 token 的合计预算（例如 Claude Opus 4.6 是 200,000）。Hermes 用它来决定何时压缩历史记录以及验证 API 请求。

**`model.max_tokens`** 是**输出上限**——模型在*单次响应*中可以生成的最大 token 数。它与你的对话历史能有多长无关。行业标准名称 `max_tokens` 是常见的混淆来源；Anthropic 的原生 API 后来将其重命名为 `max_output_tokens` 以更清晰。
...
2.  **自定义提供者按模型** — `custom_providers[].models.&lt;id&gt;.context_length`
...
8.  **[models.dev](https://models.dev)** — 社区维护的注册表，包含 100+ 提供者、3800+ 模型的提供者特定上下文长度
9.  **后备默认值** — 广泛的模型系列模式（默认 128K）

对于大多数设置，这可以开箱即用。该系统是提供者感知的——同一个模型根据服务提供者的不同可能有不同的上下文限制（例如，`claude-opus-4.6` 在 Anthropic 直连上是 1M，但在 GitHub Copilot 上是 128K）。
...
如果你使用多个自定义端点（例如，本地开发服务器和远程 GPU 服务器），你可以在 `config.yaml` 中将它们定义为命名的自定义提供者：
...
你也可以从交互式的 `hermes model` 菜单中选择命名的自定义提供者。
...
| **中文 AI 模型** | z.ai (GLM)、Kimi/Moonshot (`kimi-coding` 或 `kimi-coding-cn`)、MiniMax 或小米 MiMo（一级提供者） |

:::tip
你可以随时使用 `hermes model` 在提供者之间切换——无需重启。无论你使用哪个提供者，你的对话历史、记忆和技能都会保留。
:::
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更倾向于本地运行 Firecrawl，可以改为将 Hermes 指向一个自托管实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。

**你能得到什么：** 无需 API 密钥，无速率限制，无按页计费，完全的数据主权。

**你会失去什么：** 云版本使用 Firecrawl 专有的 "Fire-engine" 进行高级反机器人绕过（Cloudflare、CAPTCHA、IP 轮换）。自托管版本使用基本的 fetch + Playwright，因此一些受保护的网站可能会失败。搜索使用 DuckDuckGo 而不是 Google。
...
使用 OpenRouter 时，你可以控制请求如何在提供者之间路由。在 `~/.hermes/config.yaml` 中添加一个 `provider_routing` 部分：
...
配置一个备份提供者:模型，当你的主模型失败时（速率限制、服务器错误、认证失败），Hermes 会自动切换到它：
...
```
启用后，回退机制会在会话中途切换模型和提供商，而不会丢失对话内容。每个会话**最多触发一次**。
...
:::tip
回退功能仅通过 `config.yaml` 配置——没有对应的环境变量。关于其触发条件、支持的提供商以及如何与辅助任务和委派交互的完整细节，请参阅[回退提供商](/user-guide/features/fallback-providers)。
:::

## 智能模型路由

可选的廉价与强力模型路由功能，让 Hermes 在处理复杂工作时使用你的主模型，而将非常简短/简单的对话轮次发送给更廉价的模型。
...
工作原理：
- 如果某个对话轮次简短、单行，且看起来不涉及大量代码/工具/调试，Hermes 可能会将其路由到 `cheap_model`
- 如果该轮次看起来复杂，Hermes 会继续使用你的主模型/提供商
- 如果廉价路由无法顺利解析，Hermes 会自动回退到主模型

此功能设计上较为保守。它适用于快速、低风险的轮次，例如：
- 简短的事实性问题
- 快速重写
- 轻量级摘要
