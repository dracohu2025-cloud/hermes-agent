...
本页介绍如何为 Hermes Agent 设置推理提供商——从 OpenRouter 和 Anthropic 等云 API，到 Ollama 和 vLLM 等自托管端点，再到高级路由和故障转移配置。你需要至少配置一个提供商才能使用 Hermes。
...
:::info Codex 说明
OpenAI Codex 提供商通过设备代码进行身份验证（打开一个 URL，输入一个代码）。Hermes 将生成的凭据存储在其自己的认证存储 `~/.hermes/auth.json` 中，并且当存在时可以从 `~/.codex/auth.json` 导入现有的 Codex CLI 凭据。无需安装 Codex CLI。
:::

:::warning
即使使用 Nous Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）也会使用一个单独的“辅助”模型——默认是通过 OpenRouter 的 Gemini Flash。设置 `OPENROUTER_API_KEY` 会自动启用这些工具。你也可以配置这些工具使用哪个模型和提供商——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::
...
当你通过 `hermes model` 选择 Anthropic OAuth 时，Hermes 优先使用 Claude Code 自身的凭据存储，而不是将令牌复制到 `~/.hermes/.env`。这样可以保持 Claude 凭据的可刷新性。
...
**`copilot-acp` — Copilot ACP Agent 后端**。在本地生成 Copilot CLI 子进程：
...
# Alibaba Cloud / DashScope (通义千问模型)
...
:::note Z.AI 端点自动检测
当使用 Z.AI / GLM 提供商时，Hermes 会自动探测多个端点（全球、中国、编码变体）以找到接受你 API 密钥的那个。你无需手动设置 `GLM_BASE_URL`——可用的端点会被自动检测并缓存。
:::
### xAI (Grok) 提示词缓存

当使用 xAI 作为提供商（任何包含 `x.ai` 的基础 URL）时，Hermes 会自动启用提示词缓存，方法是在每个 API 请求中发送 `x-grok-conv-id` 请求头。这会将请求路由到对话会话内的同一台服务器，允许 xAI 的基础设施重用缓存的系统提示词和对话历史。
...
Ollama 默认**不**使用模型的完整上下文窗口。根据你的 VRAM，默认值如下：
...
**上下文长度：** vLLM 默认读取模型的 `max_position_embeddings`。如果该值超过你的 GPU 内存，它会报错并要求你将 `--max-model-len` 设置得更低。你也可以使用 `--max-model-len auto` 自动找到适合的最大值。设置 `--gpu-memory-utilization 0.95`（默认 0.9）以将更多上下文挤入 VRAM。
...
### SGLang —— 搭载 RadixAttention 的快速服务

[SGLang](https://github.com/sgl-project/sglang) 是 vLLM 的替代方案，具备 RadixAttention 以实现 KV 缓存复用。最适合：多轮对话（前缀缓存）、约束解码、结构化输出。
...
**工具调用：** 使用 `--tool-call-parser` 并为你模型系列选择合适的解析器：`qwen` (Qwen 2.5)、`llama3`、`llama4`、`deepseekv3`、`mistral`、`glm`。没有此标志时，工具调用会以纯文本形式返回。
...
**上下文长度 (`-c`):** 最近的构建默认值为 `0`，这会从 GGUF 元数据中读取模型的训练上下文长度。对于训练上下文为 128k+ 的模型，尝试分配完整的 KV 缓存可能会导致内存不足。请根据你的需要显式设置 `-c`（对于 Agent 使用，32k–64k 是一个不错的范围）。如果使用并行槽位 (`-np`)，总上下文长度会在槽位间分配 —— 使用 `-c 32768 -np 4` 时，每个槽位只获得 8k。
...
没有 `--jinja`，llama-server 会完全忽略 `tools` 参数。模型会尝试通过在响应文本中写入 JSON 来调用工具，但 Hermes 不会将其识别为工具调用 —— 你会看到像 `{"name": "web_search", ...}` 这样的原始 JSON 被打印为消息，而不是实际的搜索。
...
如果你在 WSL2 内运行 Hermes，而你的模型服务器在 Windows 主机上，在 WSL2 默认的 NAT 网络模式下，`http://localhost:<端口>` 将无法工作。请参考上面的 [WSL2 网络](#wsl2-networking-windows-users) 部分进行修复。
...
**修复：** 为 Agent 使用设置至少 **32,768 个 token** 的上下文。查看上面每个服务器部分的具体标志。

#### 启动时显示“Context limit: 2048 tokens”

Hermes 会自动从你服务器的 `/v1/models` 端点检测上下文长度。如果服务器报告的值很低（或根本没有报告），Hermes 会使用模型声明的限制，这可能是错误的。
...
BlockRunAI 的 [ClawRouter](https://github.com/BlockRunAI/ClawRouter) 是一个本地路由代理，根据查询复杂度自动选择模型。它根据 14 个维度对请求进行分类，并路由到能处理任务的最便宜模型。通过 USDC 加密货币支付（无需 API 密钥）。
...
通过 `hermes model` → 自定义端点或在 `config.yaml` 中配置这些服务：
...
:::note 两个设置，容易混淆
**`context_length`** 是**总上下文窗口** — 输入*和*输出 token 的合计预算（例如 Claude Opus 4.6 的 200,000）。Hermes 用它来决定何时压缩历史记录以及验证 API 请求。

**`model.max_tokens`** 是**输出上限** — 模型在*单次响应*中可以生成的最大 token 数。它与你的对话历史能有多长无关。行业标准名称 `max_tokens` 是常见的混淆来源；Anthropic 的原生 API 后来为了清晰将其重命名为 `max_output_tokens`。
当自动检测获取的窗口大小不正确时，设置 `context_length`。
仅当你需要限制单个响应的长度时，才设置 `model.max_tokens`。
:::

Hermes 使用一个多源解析链来检测你的模型和提供商的正确上下文窗口：
...
8.  **[models.dev](https://models.dev)** — 社区维护的注册表，包含 100+ 提供商下 3800+ 个模型的提供商特定上下文长度
...
如果你需要处理多个自定义端点（例如，本地开发服务器和远程 GPU 服务器），你可以在 `config.yaml` 中将它们定义为命名的自定义提供商：
...
在会话中，你可以使用三重语法在它们之间切换：

```
/model custom:local:qwen-2.5       # 使用 "local" 端点与 qwen-2.5
/model custom:work:llama3-70b      # 使用 "work" 端点与 llama3-70b
...
默认情况下，Hermes 使用 [Firecrawl 云 API](https://firecrawl.dev/) 进行网页搜索和抓取。如果你更倾向于本地运行 Firecrawl，可以改为将 Hermes 指向一个自托管的实例。完整的设置说明请参见 Firecrawl 的 [SELF_HOST.md](https://github.com/firecrawl/firecrawl/blob/main/SELF_HOST.md)。
...
激活后，后备模型会在会话中途切换模型和提供商，而不会丢失你的对话。它**每个会话最多触发一次**。
...
:::tip
后备模型配置**仅**通过 `config.yaml` 进行——没有相关的环境变量。关于它何时触发、支持的提供商以及它如何与辅助任务和委托交互的完整细节，请参阅 [后备提供商](/user-guide/features/fallback-providers)。
:::
## 智能模型路由

可选的廉价/强力模型路由功能，让 Hermes 可以将复杂任务留给主模型处理，同时将非常简短/简单的对话轮次发送给更廉价的模型。
